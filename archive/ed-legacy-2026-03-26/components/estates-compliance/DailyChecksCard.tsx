...
export function DailyChecksCard({ onCheckComplete }: DailyChecksCardProps) {
  ...
  const fetchDailyCheckStatus = useCallback(
    async (signal?: AbortSignal) => {
      ...
      const timeoutId = setTimeout(
        () => controller.abort("Daily checks fetch timed out"),
        30000,
      );
      ...
      try {
        ...
        const response = await fetch(
          `/api/estates-compliance/daily-checks?organizationId=${organizationId}&date=${getTodayDate()}`,
          {
            headers,
  ...
  const sortedStatusEntries = Object.entries(statuses).sort(
    ([idA, sA], [idB, sB]) => {
  ...
  const pendingCount = sortedStatusEntries.filter(
    ([_, s]) => !s.completed,
  ).length;
    ...
    <Card className="border-2 border-primary/20 bg-gradient-to-br from-primary/5 to-background overflow-hidden relative">
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          >
            {allCompleted ? "All Complete" : `${pendingCount} pending`}
          </Badge>
        </div>
      </CardHeader>
        ...
        {sortedStatusEntries.map(([id, status]) => (
          ...
          >
            <div className="flex items-start gap-4">
              <div
                className={`text-4xl filter drop-shadow-sm ${celebrating === id ? "animate-bounce" : ""}`}
              >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <h3 className="font-bold text-gray-800">
                    {getRoutineDisplayName(id, status)}
                  </h3>
                  {status.completed && status.result && (
                    <Badge
                      variant="outline"
                      className={`text-[10px] uppercase font-bold px-1.5 h-5 leading-none ${
                ...
                {status.completed && status.result ? (
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-bold text-muted-foreground whitespace-nowrap">
                      {Math.round(
                        (status.result.passed / status.result.total) * 100,
                      )}
                      %
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-3 text-[10px] font-bold text-muted-foreground">
                    <div className="flex items-center gap-1">
                      ...
                      {status.deadline_time
                        ? `Deadline: ${status.deadline_time.slice(0, 5)}`
                        : "No deadline"}
                    </div>
                    {(status.items_count ?? 0) > 0 && (
                      <div className="flex items-center gap-1">
              ...
              <Button
                ...
                className={`transition-all rounded-xl font-bold shadow-sm ${
                  status.completed
