...
export class EdAPIClient {
  ...
  async chat(
    ...
    try {
      ...
      console.log("[EdAPIClient] chat: sending with image=", !!image, image ? `(${image.length} bytes)` : "");
  ...
  captureFrame(): string | null {
    if (!this.screenVideo || !this.screenStream?.active) {
      console.log("[EdAPIClient] captureFrame: no active screen share");
      return null;
    }
    ...
    try {
      ...
      console.log("[EdAPIClient] captureFrame: success, size=", dataUrl.length, "bytes");
      return dataUrl;
    } catch (e) {
      console.error("[EdAPIClient] captureFrame: error", e);
      return null;
    }
  }
