...
// Use global THREE if available (to prevent multiple instances), otherwise import
import * as THREE from 'three';

// Expose to window for other modules to use
if (typeof window !== 'undefined' && !(window as any).THREE) {
  (window as any).THREE = THREE;
}

// Use global THREE if set
const _THREE = (typeof window !== 'undefined' ? (window as any).THREE : null) || THREE;
...
export class Particle3D {
  ...
  private createSun(): void {
    ...
    this.sun = new _THREE.Points(geometry, material);
  ...
  private createPlanets(): void {
    PLANETS.forEach((planetDef, index) => {
      ...
      const geometry = new _THREE.SphereGeometry(planetRadius, 32, 32);
      const material = new _THREE.MeshBasicMaterial({
        color: new _THREE.Color(planetDef.color[0], planetDef.color[1], planetDef.color[2]),
        transparent: true,
        opacity: 1.0,
      });

      const planet = new _THREE.Mesh(geometry, material);
