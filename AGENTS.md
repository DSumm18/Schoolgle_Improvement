...
- Trust Assessor product UI is locked unless the product owner explicitly approves a product change. Do not add bespoke school-specific UI, colour changes, layouts, demo panels, or one-off visual components to make a demo work. Demonstrations must use the existing product components and proper data paths. Demo/illustrative data is allowed only when it is inserted through the normal capture/import structure and clearly labelled as demo data that will be replaced by the school’s own submitted data if they take the product.
...
**Non-negotiable product rule**: Schoolgle must only send school/customer data to approved provider families: OpenAI, Anthropic, Google, Meta Llama, Mistral, and Microsoft. Do not add models from non-approved provider families unless the product owner has completed and documented a DPA, regional transfer, and GDPR review first.
...
| `anthropic/claude-3.5-sonnet`      | Premium synthesis/SEF quality  | Use only when justified |

**To change models**: Update the central policy in `apps/platform/src/lib/ai/model-policy.ts`, update `MODEL_CONFIG` in `ai-evidence-matcher.ts`, and document rationale in `docs/AI_MODELS.md`.
...
| Auto memory           | `.Codex/projects/C--Git-Schoolgle-Improvement/memory/` |

---

## Memory System

This repository has an auto memory system that persists information across conversations at `.Codex/projects/C--Git-Schoolgle-Improvement/memory/`.
...
- Anything duplicating AGENTS.md instructions
...
```bash
# Search memory files
Grep with pattern="<search term>" path=".Codex/projects/C--Git-Schoolgle-Improvement/memory/" glob="*.md"
