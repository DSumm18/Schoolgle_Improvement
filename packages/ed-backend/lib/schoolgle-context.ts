...
export interface EvidenceSummary {
  ...
  totalMatches: number;
  categoryCoverage: { [categoryId: string]: number };
  lastScanned?: string;
}
 ...
 */
export async function getSchoolgleContext(
  supabase: SupabaseClient,
  organizationId: string
): Promise<SchoolgleContextData> {
  try {
    ...
    const assessments: Assessment[] = (assessmentsData || []).map((a: any) => ({
      id: a.id,
    ...
    const recentActivity: Activity[] = (activityData || []).map((a: any) => ({
      id: a.id,
    ...
    const categoryCoverage: { [categoryId: string]: number } = {};
