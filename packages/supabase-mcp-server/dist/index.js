...
if (!CONNECTION_STRING) {
    throw new Error("Missing read-only database connection. Set SUPABASE_DB_URL, DATABASE_URL, or POSTGRES_URL.");
}
const pool = new Pool({
...
async function runAuditQuery(sql, params = []) {
...
async function getOverview() {
    const [rls, policies, privileges, sensitiveColumns, buckets, functions] = await Promise.all([
        getRlsStatus(),
        getPolicies(),
        getTablePrivileges(),
        getSensitiveColumns(),
    ...
    const securityDefinerFunctions = functions.filter((row) => row.security_definer === true);
    return {
    ...
    tools: [
        {
            ...
            description: "Return a metadata-only summary of RLS, policies, privileges, sensitive columns, storage buckets, and security-definer functions.",
            inputSchema: { type: "object", properties: {} },
        },
        {
            name: "list_rls_status",
            description: "List public/storage tables and whether Row Level Security is enabled. Returns estimated row counts only.",
            inputSchema: { type: "object", properties: {} },
        },
        {
            name: "list_policies",
        ...
        },
        {
            ...
            description: "List table privileges granted to anon, authenticated, service_role, or public roles.",
            inputSchema: { type: "object", properties: {} },
        },
        {
            name: "list_sensitive_columns",
            description: "List likely sensitive columns by name pattern without returning row data.",
            inputSchema: { type: "object", properties: {} },
        },
        {
            name: "list_storage_buckets",
            description: "List Supabase storage bucket configuration, including public/private status.",
            inputSchema: { type: "object", properties: {} },
        },
        {
            name: "list_function_security",
            description: "List public/storage functions, security-definer status, and execute grantees without function bodies.",
            inputSchema: { type: "object", properties: {} },
        },
    ],
}));
