...
export async function handleRiddorFill(
  ...
  try {
    ...
    const requiredFields = ['incidentDate', 'incidentType', 'personName', 'injuryNature'];
    ...
    const { data: run, error: runError } = await supabase.rpc('create_rpa_run', {
      p_skill_id: skill.id,
      p_user_id: userId,
      p_school_id: orgId,
    ...
    const severity = isFatal ? '🚨 FATAL' : incidentDetails.daysAwayFromWork && incidentDetails.daysAwayFromWork > 7 ? '⚠️ Over 7-day incapacitation' : '📋 Reportable injury';
...
[Review and Submit](${approvalUrl})`,
    ...
    return {
      success: false,
      requiresApproval: false,
      message: 'I encountered an error preparing the RIDDOR report. Please try again.',
...
export async function handleSafeguardingForm(
  ...
  try {
    ...
    const { data: redFlagCheck } = await supabase.rpc('check_form_text_red_flags', {
    ...
    if (hasRedFlags) {
      ...
      suggestedWording = {
        warning: 'Your description may need adjustment for clarity and professionalism.',
        flags: matchedFlags.map((f: any) => `- **${f.type}**: ${f.explanation}`).join('\n'),
      };
    }
...
${hasRedFlags ? `**⚠️ Review Needed:**
${suggestedWording?.flags}

The DSL may contact you to clarify these points before submitting to the LA.` : '**Next:** The DSL will review and may contact you for clarification before formal submission.'}
...
**Reference:** #${concern?.id?.substring(0, 8)}`,
    ...
    return {
      success: false,
      requiresApproval: false,
      message: 'I encountered an error logging your concern. Please speak to the DSL directly.',
      error: error instanceof Error ? error.message : 'Unknown error',
    };
  }
}
...
Which section would you like help with?`,
    ...
    return {
      success: false,
      requiresApproval: false,
      message: 'I encountered an error providing EHCP guidance. Please try again.',
      error: error instanceof Error ? error.message : 'Unknown error',
    };
  }
}
  ...
  try {
    ...
    if (!org || org.local_authority !== 'Bradford') {
      return {
        success: false,
        requiresApproval: false,
        message: 'This automation is only available to Bradford schools. Please contact support if you believe this is an error.',
      };
    }
...
export const FORM_SKILLS = [
  {
    ...
    description: 'Prepare RIDDOR injury reports with human review before HSE submission',
    category: 'Safety',
  ...
  },
  {
    ...
    description: 'Step-by-step guidance for EHCP applications with legal context',
    category: 'SEND',
  ...
  switch (skillId) {
    case 'fill_riddor_injury':
      return {
        ...
        incidentTime: { type: 'string', description: 'Time of the incident (HH:MM)' },
        incidentType: { type: 'string', description: 'Type of incident (e.g., "Injury", "Death", "Dangerous occurrence")' },
        personName: { type: 'string', description: 'Name of the injured person' },
        personRole: { type: 'string', description: 'Role (e.g., "Employee", "Pupil", "Visitor")' },
        ...
        immediateCause: { type: 'string', description: 'What caused the incident' },
        daysAwayFromWork: { type: 'number', description: 'Number of days away from work' },
        fatal: { type: 'boolean', description: 'Whether the incident was fatal' },
      };
    case 'fill_safeguarding':
      return {
        childName: { type: 'string', description: 'Name of the child (optional)' },
        concerns: { type: 'string', description: 'Description of the concerns' },
        incidentDate: { type: 'string', description: 'Date of the incident (if applicable)' },
        immediateRisk: { type: 'boolean', description: 'Whether there is immediate risk to the child' },
      };
    case 'fill_send_ehcp':
      return {
        question: { type: 'string', description: 'The user\'s question or drafted text' },
        currentField: { type: 'string', description: 'The current field they\'re working on (optional)' },
      };
    case 'report_bradford_sickness':
      return {
        dateRange: { type: 'string', description: 'Date range for the report (default: last 7 days)' },
      };
    default:
      return {};
  }
...
export async function executeFormSkill(
  ...
  switch (skillId) {
    ...
    case 'fill_safeguarding':
      return await handleSafeguardingForm(context, parameters);
    case 'fill_send_ehcp':
      return await handleSendEhcpGuidance(context, parameters.question, parameters.currentField);
    case 'report_bradford_sickness':
