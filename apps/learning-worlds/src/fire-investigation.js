 ...
 */
export function mountFireInvestigation(container,{draft={},record=()=>{},help=new Set(),save=()=>{},finish=()=>{},reducedMotion=false,speak=()=>{}}={}){
 ...
 const setPhase=phase=>{if(disposed)return;s.phase=phase;s.feedback='';persist();draw(true);};
 ...
 function shell(title,body,step,readText){
 ...
 function beginRun(transfer){
  ...
  const complete=skipped=>{
   if(disposed||token!==runToken)return;cancelTimers();
 ...
 function draw(focus=false){
  ...
  if(s.phase==='predict'){
   ...
   shell('Has your thinking changed?',`<p>Your first prediction: <strong>${safe(PREDICTIONS[s.prediction])}</strong></p><p>Your model reached ${s.practiceResult.count} houses instead of 5. How does that compare with your first idea?</p><div class="fi-choice-list">${Object.entries(REFLECTIONS).map(([id,text])=>`<button type="button" data-fi-reflection="${id}">${text}</button>`).join('')}</div><p class="fi-small">This is your reflection, not a score. Next, try a new layout without this explanation beside it.</p>`,4);
 ...
 draw(false);
 return {dispose(){if(disposed)return;cancelTimers();if(s.phase==='running')s.phase='place';if(s.phase==='transfer-running')s.phase='transfer';disposed=true;}};
}
