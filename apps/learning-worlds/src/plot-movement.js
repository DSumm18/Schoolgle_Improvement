...
export function mountPlotMovement({scene,toolbar,world,enabled}){
 ...
 const inControls=target=>target instanceof Element&&(scene.contains(target)||toolbar.contains(target)||Boolean(target.closest('#plot-nearby')));
 ...
 function stop(){keys.clear();pointer=null;pointerDirection=null;buttonDirection=null;clearTimeout(pulse);pulse=null;toolbar.querySelectorAll('[data-plot-move]').forEach(b=>b.classList.remove('is-moving'));world()?.stopExplorer();}
 ...
 for(const button of toolbar.querySelectorAll('[data-plot-move]')){
  const name=button.dataset.plotMove;
 ...
 toolbar.querySelector('#plot-walk-focus').addEventListener('click',()=>{stop();scene.focus({preventScroll:true});focusExplorer();},options);
