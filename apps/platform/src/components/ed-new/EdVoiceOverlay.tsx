 ...
 * Ed Voice Overlay
 *
 * Full-screen voice chat overlay using Gemini Live API.
 * Integrates with the main Ed chatbot to provide real-time voice conversation.
 */
...
import { useGeminiLive, VoiceState } from '@/components/ed-voice/useGeminiLive';

export interface EdVoiceOverlayProps {
  onClose: () => void;
  onTranscript?: (text: string) => void;
}

export default function EdVoiceOverlay({ onClose, onTranscript }: EdVoiceOverlayProps) {
  ...
  // Start voice when overlay mounts
  useEffect(() => {
    start();
    return () => {
      stop();
  ...
  const getStateText = () => {
    switch (state) {
      ...
      case 'speaking': return 'Ed is speaking...';
  ...
  const getStateColor = () => {
    switch (state) {
      case 'idle': return 'bg-slate-500/30';
      case 'connecting': return 'bg-amber-500/30';
      case 'listening': return 'bg-blue-500/30';
      case 'speaking': return 'bg-emerald-500/30';
  ...
  return (
    <div
      ref={overlayRef}
      className="fixed inset-0 z-[10000] flex flex-col items-center justify-center bg-slate-900/95 backdrop-blur-sm animate-in fade-in duration-300"
    >
      {/* Close button */}
      <button
        onClick={handleClose}
      ...
      <div className="mb-8 text-center">
        ...
        <h2 className="text-2xl font-semibold text-white mb-2">Ed</h2>
        <p className="text-sm text-white/70">{getStateText()}</p>
      </div>
      ...
      <p className="mt-6 text-xs text-white/30">
        Press <kbd className="rounded bg-white/10 px-1.5 py-0.5 font-mono text-xs">Esc</kbd> to end the call
      </p>
    </div>
  );
}
