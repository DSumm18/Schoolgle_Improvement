...
import type { StaffMember, StaffRoleCategory, StaffModuleAccess } from '@/lib/staff-directory';
    ...
    const getInitialFormData = () => {
        if (staff && mode === 'edit') {
            return {
                ...
                role_category: (staff.role_category || 'class_teacher') as StaffRoleCategory,
                is_super_user: staff.is_super_user || false,
                is_active: staff.is_active !== undefined ? staff.is_active : true,
                accessible_modules: (staff.accessible_modules || []) as StaffModuleAccess[],
            };
        }
        return {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
                        ...
                        <div>
                            ...
                            <Select
                                ...
                                onValueChange={(value) =>
                                    setFormData({ ...formData, salutation: value === 'none' ? null : value })
                                }
                            >
                                ...
                                {MODULE_OPTIONS.map((module) => (
                                    ...
                                    >
                                        <Checkbox
                                            ...
                                            checked={formData.accessible_modules.includes(module.value)}
                            ...
                            {formData.accessible_modules.length > 0 && (
                                <div className="flex flex-wrap gap-1 pt-2">
                                    {formData.accessible_modules.map((mod) => (
                                        <Badge
                                            key={mod}
                                            className={MODULE_COLORS[mod]}
                                            onClose={() => toggleModule(mod)}
                                        >
                                            {MODULE_OPTIONS.find((m) => m.value === mod)?.label}
                                        </Badge>
                                    ))}
                                </div>
                            )}
