...
export default function SiamsFrameworkView({
  ...
  const [edContext, setEdContext] = useState({
    strand: "",
    rating: "",
    evidenceCount: 0,
  });
  ...
  const overallScores = useMemo(
    () => calculateOverallSiamsReadiness(assessments as any),
    [assessments],
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10 animated-mesh min-h-screen">
                  ...
                  {strand.inspectionQuestions.map((question) => (
                    <SubcategoryAssessment
                      ...
                      subcategory={{
                        ...
                        evidenceRequired: question.evidenceRequired.map(
                          (er, idx) => ({ id: `${idx}`, name: er }),
                        ),
                      }}
                      assessment={assessments[question.id] || {}}
                      evidenceMatches={Object.fromEntries(
                        question.evidenceRequired.map((er, idx) => {
                          const areas = SIAMS_TO_EVIDENCE[strand.id] || [
                            strand.id,
                          ];
                          const matches = areas.flatMap(
        ...
        {actionModal.isOpen && (
          <ActionModal
            ...
            subCategoryName={
              SIAMS_FRAMEWORK.flatMap((s) => s.inspectionQuestions).find(
                (q) => q.id === actionModal.subId,
              )?.question || ""
            }
        ...
        {showEdPanel && (
          <EdAnalysisPanel
            ...
            selectedCategory={edContext.strand}
