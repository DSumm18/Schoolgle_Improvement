...
const HEADER_COLOR_MAP: Record<string, string> = {
  ...
  orange:
    "bg-white border-l-4 border-orange-500 text-orange-900 hover:bg-orange-50",
  violet:
    "bg-white border-l-4 border-violet-500 text-violet-900 hover:bg-violet-50",
...
export default function OfstedFrameworkView({
  ...
  const auth = useAuth();
  const { user, organization, signInWithGoogle, signInWithMicrosoft } = auth;
  const accessToken = (auth as any).accessToken;
  const providerId = (auth as any).providerId;
  ...
  const [currentActionSubId, setCurrentActionSubId] = useState<string | null>(
    null,
  );
  const [currentActionEvidence, setCurrentActionEvidence] = useState<
    string | null
  >(null);
  ...
  const handleEdAction = (draftAction: any) => {
    ...
    if (!matchingSubcategory) {
      console.warn(
        "Could not find matching subcategory for action:",
        action.category,
      );
      return;
    }
  ...
  const { userScore: overallUserScore, aiScore: overallAIScore } =
  ...
  const handleSaveAction = (action: ActionItem) => {
    ...
    let newActions;
    if (editingAction) {
      newActions = currentActions.map((a: ActionItem) =>
  ...
  const handleScanEvidence = async () => {
    if (!accessToken || !providerId) {
      setScanProgress({
        status: "error",
        message: "Please reconnect your account to enable scanning.",
      });
      return;
    }
    ...
    try {
      ...
      if (data.assessmentUpdates) {
        ...
        Object.entries(data.assessmentUpdates).forEach(
          ([subId, update]: [string, any]) => {
  ...
  return (
    <div className="space-y-4">
          ...
          <div className="flex items-start gap-4 pr-8">
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-blue-900 text-lg mb-2">
                About the Education Inspection Framework (EIF)
              </h3>
              <p className="text-blue-800 text-sm leading-relaxed mb-3">
                The EIF is the framework used by Ofsted to inspect schools in
                England. This tool helps you track your school's readiness
                against the key categories of Quality of Education, Behaviour &
                Attitudes, Personal Development, and Leadership & Management.
                For each area, you can self-assess, add evidence, create action
                plans, and see how AI analysis compares to your judgements.
              </p>
              <a
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-600">
              Track evidence and readiness against the Education Inspection
              Framework (EIF)
            </p>
          </div>
          <div className="flex items-center gap-6">
                    ...
                    <div className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-xl border border-gray-200 z-20 p-4 animate-in fade-in slide-in-from-top-2">
                      ...
                      <div className="mb-4">
                        ...
                        <div className="flex items-center gap-2 text-sm text-gray-700 bg-gray-50 p-2 rounded border border-gray-200">
                          {providerId === "microsoft.com" ? (
                            <>
                              <span className="text-blue-600">OneDrive</span>{" "}
                              Connected
                            </>
                          ) : (
                            <>
                      ...
                      <div className="mb-4">
                        ...
                        <p className="text-[10px] text-gray-400 mt-1">
                          Leave as "root" to scan everything
                        </p>
                      </div>
            ...
            <div className="flex gap-6 border-l border-gray-200 pl-6">
              <div className="text-right">
                ...
                <div
                  className={`text-3xl font-bold ${getScoreColor(overallUserScore)}`}
                >
                  {overallUserScore}%
                </div>
              </div>
              <div className="text-right">
                ...
                <div
                  className={`text-3xl font-bold ${getScoreColor(overallAIScore)}`}
                >
                  {overallAIScore}%
                </div>
              </div>
            ...
            <div className="flex-1">
              ...
              {scanProgress.stats && (
                <div className="mt-2 text-sm grid grid-cols-2 gap-x-8 gap-y-1">
                  <span>
                    Files Processed: <b>{scanProgress.stats.processedFiles}</b>
                  </span>
                  <span>
                    Evidence Matches:{" "}
                    <b>{scanProgress.stats.evidenceMatches}</b>
                  </span>
                  <span>
                    Skipped (Unchanged):{" "}
                    <b>{scanProgress.stats.skippedFiles}</b>
                  </span>
                  <span>
                    Failed: <b>{scanProgress.stats.failedFiles}</b>
                  </span>
                </div>
              )}
            </div>
        ...
        <div className="space-y-3">
          ...
          {OFSTED_FRAMEWORK.map((category) => {
            ...
            return (
              <div
                key={category.id}
                className={`mb-4 border rounded-lg overflow-hidden transition-all duration-200 ${expandedCategories.has(category.id) ? "shadow-md" : "shadow-sm"} ${HEADER_COLOR_MAP[category.color || "gray"].replace("hover:", "")}`}
              >
                {/* Category Header */}
                <div
                  className={`transition-colors ${HEADER_COLOR_MAP[category.color || "gray"]}`}
                >
                  <div className="flex items-center justify-between w-full p-4">
                    ...
                    <div className="flex items-center gap-4 mr-4">
                      ...
                      <div className="text-right">
                        ...
                        <div
                          className={`text-lg font-bold ${getScoreColor(userScore)}`}
                        >
                          {userScore}%
                        </div>
                      </div>
                      <div className="text-right border-l border-gray-200 pl-4">
                        ...
                        <div
                          className={`text-lg font-bold ${getScoreColor(aiScore)}`}
                        >
                          {aiScore}%
                        </div>
                      </div>
                  ...
                  <div className="p-4 bg-white space-y-6 border-t border-gray-100">
                    ...
                    {getEvidenceCountForCategory(category.id) > 0 && (
                      <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-4">
                        ...
                        <div className="space-y-2">
                          {getEvidenceForCategory(category.id).map(
                            (evidence, idx) => (
                              <div
                                key={idx}
                    ...
                    {category.subcategories.map((sub) => {
                      ...
                      const aiRationale =
                        ...
                        "AI analysis pending evidence scan.";
                          ...
                          <div className="flex justify-between items-start mb-4">
                            ...
                            <div className="flex gap-4 ml-4">
                              <div className="text-right">
                                ...
                                <div
                                  className={`px-3 py-1.5 rounded-md text-sm font-medium border ${getGradeColor(aiRating)}`}
                                >
                              ...
                              <div className="text-right">
                                ...
                                <select
                                  value={schoolRating}
                                  onChange={(e) =>
                                    handleRatingChange(sub.id, e.target.value)
                                  }
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                              <div className="flex items-center justify-between mb-2">
                                ...
                                <span
                                  className={`text-xs font-bold px-2 py-0.5 rounded ${hasGaps ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
                                >
                              ...
                              <ul className="space-y-2">
                                ...
                                {sub.evidenceRequired.map((evidence, idx) => (
                                  <li
                                    key={evidence.id || idx}
                                    className="flex items-start justify-between group text-sm p-2 rounded hover:bg-gray-50"
                                  >
                                    ...
                                    {actions.map((action) => (
                                      ...
                                      >
                                        <div className="flex justify-between items-start mb-1">
                                          <div className="flex items-center gap-2">
                                            <span
                                              className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase border ${getPriorityColor(action.priority)}`}
                                            >
                                              {action.priority}
                                            </span>
                                          ...
                                          <button
                                            onClick={() =>
                                              handleActionEdit(sub.id, action)
                                            }
                                        ...
                                        <div className="flex items-center gap-4 text-xs text-gray-500">
                                          ...
                                          <span
                                            className={`px-2 py-0.5 rounded text-[10px] font-medium ${action.status === "completed" ? "bg-green-100 text-green-700" : action.status === "in_progress" ? "bg-yellow-100 text-yellow-700" : "bg-gray-100 text-gray-700"}`}
                                          >
      ...
      <EvidenceModal
        ...
        onClose={() =>
          setEvidenceModalData({ ...evidenceModalData, isOpen: false })
        }
