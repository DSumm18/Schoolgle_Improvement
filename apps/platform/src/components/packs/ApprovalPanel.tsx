...
interface ApprovalPanelProps {
  pack: Pack;
  userRole: "admin" | "headteacher" | "governor" | "staff" | "sbm";
  userId: string;
...
export default function ApprovalPanel({
  ...
  const handleAction = async (
    action: "approve" | "request-changes" | "reject" | "submit",
  ) => {
    setProcessing(true);
    try {
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mt-1">
            Current Status: {pack.status.replace("_", " ")}
          </p>
        </div>
        <div
      ...
      {pack.status === "draft" || pack.status === "changes_requested" ? (
        <div className="space-y-6">
          <p className="text-sm text-gray-500 font-medium leading-relaxed">
            Ready for formal board review? Submission will snapshot the current
            version and notify the governors.
          </p>
          <button
        ...
        <div className="text-center py-6">
          ...
          <p className="text-sm text-gray-400 font-medium mt-1">
            This pack is approved and locked. You can now proceed to export.
          </p>
        </div>
      )}
    </div>
