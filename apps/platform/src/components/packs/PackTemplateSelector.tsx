...
export default function PackTemplateSelector({
  onSelect,
  onClose,
}: PackTemplateSelectorProps) {
  const [templates, setTemplates] = useState<PackTemplate[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedTemplate, setSelectedTemplate] = useState<PackTemplate | null>(
    null,
  );
      ...
      <div className="p-8 border-b border-gray-100 flex justify-between items-center bg-gradient-to-r from-blue-50 to-indigo-50">
        <div>
          ...
          <p className="text-sm text-gray-500 font-medium mt-1 uppercase tracking-widest">
            Navigator Governor Pack Lite (v1.0)
          </p>
        </div>
        <button
          onClick={onClose}
            ...
            >
              ...
              <p className="text-xs text-gray-500 mt-2 line-clamp-2 font-medium">
                {template.description ||
                  "Standard reporting template for governors."}
              </p>
          ...
          {selectedTemplate ? (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              <div className="space-y-6">
                <h4 className="text-xs font-black text-gray-400 uppercase tracking-[0.2em] border-b border-gray-100 pb-2">
                  Included Sections
                </h4>
                {selectedTemplate.sections.map((section, idx) => (
                  <div key={idx} className="flex gap-4 group">
                    <div className="text-xs font-black text-blue-300 w-4 pt-1">
                      {idx + 1}.
                    </div>
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        {section.evidence_categories.map((cat, ci) => (
                          <span
                            key={ci}
                            className="text-[9px] font-bold uppercase tracking-wider bg-gray-100 text-gray-500 px-2 py-0.5 rounded"
                          >
                            {cat}
                          </span>
                        ))}
              ...
              <div className="mt-12">
                <button
                  ...
                  className="w-full bg-blue-600 hover:bg-blue-700 text-white p-5 rounded-2xl font-black text-lg flex items-center justify-center gap-3 shadow-md transition-all active:scale-[0.98] group"
                >
                  START THIS PACK
                  <ChevronRight
                    size={24}
            ...
            <div className="h-full flex flex-col items-center justify-center text-center px-12 opacity-50">
              ...
              <p className="text-sm text-gray-500 font-medium mt-2">
                Pick a reporting framework from the left to see the structure
