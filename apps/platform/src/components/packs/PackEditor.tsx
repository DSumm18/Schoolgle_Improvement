...
export default function PackEditor({
  ...
  const currentWordCount =
    activeSection?.content.split(/\s+/).filter((w) => w.length > 0).length || 0;
                  ...
                  <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {section.evidence_ids.map((id) => (
                        <div
                          key={id}
                          className="text-xs font-bold text-blue-600 flex items-center gap-2"
                        >
                          <CheckCircle2 size={12} />{" "}
                          {linkedEvidence[id]?.title ||
                            "Linked Evidence ID: " + id.slice(0, 8)}
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      <aside className="w-80 bg-white border-r border-gray-200 flex flex-col shadow-sm z-10">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          <div className="px-2 mb-4">
            <h1
              className="text-xl font-black text-gray-900 leading-tight truncate"
              title={pack.title}
            >
              {pack.title}
            </h1>
          ...
          <div className="space-y-1">
            <span className="px-2 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">
              Actions
            </span>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between">
                    <span
                      className={`text-[10px] font-black ${isActive ? "text-blue-600" : "text-gray-300"}`}
                    >
                  ...
                  <h3
                    className={`text-sm font-bold mt-1 ${isActive ? "text-gray-900" : "text-gray-500"}`}
                  >
                    {section.title}
                  </h3>
            ...
            <div className="flex-1 flex flex-col gap-8">
              ...
              <div className="flex flex-col flex-1 gap-2">
                <div className="flex justify-between items-end">
                  ...
                  <span
                    className={`text-[10px] font-black uppercase ${currentWordCount > 500 ? "text-amber-600" : "text-gray-400"}`}
                  >
                ...
                <div className="flex justify-between items-center mb-6">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 font-medium mt-1">
                      High-fidelity evidence attached to this section for
                      governors.
                    </p>
                  </div>
                  {!isLocked && (
                  ...
                  {activeSection.evidence_ids.length > 0 ? (
                    activeSection.evidence_ids.map((id) => {
                      ...
                      return (
                        <div
                          key={id}
                          className="group relative bg-gray-50 rounded-2xl p-4 border border-transparent hover:border-blue-200 hover:bg-blue-50 transition-all"
                        >
                            ...
                            {!isLocked && (
                              <button
                                onClick={() => {
                                  handleUpdateSection(activeSection.id, {
                                    evidence_ids:
                                      activeSection.evidence_ids.filter(
                                        (eid) => eid !== id,
                                      ),
                                  });
                                }}
      ...
      <aside className="w-96 bg-gray-900 p-8 flex flex-col gap-6 text-white overflow-y-auto">
        ...
        <div className="mt-8">
          ...
          <div className="bg-white/5 rounded-2xl p-6 border border-white/10 italic text-xs text-gray-300 leading-relaxed">
            "Snapshot v{(pack as any).current_version || 1} shows strong
            alignment with latest governance reporting standards."
          </div>
        </div>
      </aside>
      ...
      {isEvidencePickerOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <EvidencePicker
            ...
            onSelect={(ids) =>
              handleUpdateSection(activeSectionId, { evidence_ids: ids })
            }
