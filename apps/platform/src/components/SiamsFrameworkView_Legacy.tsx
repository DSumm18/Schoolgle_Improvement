...
const STRAND_COLOR_MAP: Record<string, string> = {
  purple:
    "bg-white border-l-4 border-purple-500 text-purple-900 hover:bg-purple-50",
  ...
  emerald:
    "bg-white border-l-4 border-emerald-500 text-emerald-900 hover:bg-emerald-50",
};
...
export default function SiamsFrameworkView({
  ...
  const [currentActionQuestionId, setCurrentActionQuestionId] = useState<
    string | null
  >(null);
  const [currentActionEvidence, setCurrentActionEvidence] = useState<
    string | null
  >(null);
  const [currentStrandName, setCurrentStrandName] = useState<
    string | undefined
  >(undefined);
  const [editingAction, setEditingAction] = useState<
    SiamsActionItem | undefined
  >(undefined);

  // Calculate Overall Scores
  const { userScore: overallUserScore, totalEvidence } =
    calculateOverallSiamsReadiness(assessments);
  ...
  const handleSaveAction = (action: any) => {
    ...
    let newActions;
    if (editingAction) {
      newActions = currentActions.map((a: any) =>
    ...
    <div className="space-y-4">
          ...
          <div className="flex items-start gap-4 pr-8">
            ...
            <div className="flex-1">
              ...
              <p className="text-purple-800 text-sm leading-relaxed mb-3">
                ...
                evaluates how well a church school's Christian vision enables
                pupils and adults to flourish. The framework focuses on 7
                strands covering vision, curriculum, character, community,
                dignity, worship, and RE.
              </p>
              <a
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-6">
            <div className="text-right">
              ...
              <div
                className={`text-2xl font-bold ${getScoreColor(overallUserScore)}`}
              >
                {overallUserScore}%
              </div>
            </div>
        ...
        <div className="space-y-3">
          {SIAMS_FRAMEWORK.map((strand) => {
            const { userScore, evidenceCount } = calculateStrandReadiness(
              strand.id,
              assessments,
            );
            ...
            return (
              <div
                key={strand.id}
                className={`mb-4 border rounded-lg overflow-hidden transition-all duration-200 ${expandedStrands.has(strand.id) ? "shadow-md" : "shadow-sm"}`}
              >
                {/* Strand Header */}
                <div
                  className={`transition-colors ${STRAND_COLOR_MAP[strand.color] || STRAND_COLOR_MAP["purple"]}`}
                >
                    ...
                    {localEvidenceCount > 0 && (
                      <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-4">
                        ...
                        <div className="space-y-2">
                          {getEvidenceForStrand(strand.id).map(
                            (evidence, idx) => (
                              <div
                                key={idx}
                        ...
                        >
                          ...
                          <div className="mb-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {Object.entries(SIAMS_RATINGS).map(
                                ([key, value]) => (
                                  <button
                                    key={key}
                                    onClick={() =>
                                      handleRatingChange(question.id, key)
                                    }
                          ...
                          <div className="bg-gray-50 rounded-lg p-3">
                            ...
                            <ul className="space-y-2">
                              {question.evidenceRequired.map(
                                (evidence, idx) => (
                                  <li
                                    key={idx}
                                    className="flex items-start justify-between group text-sm p-2 rounded hover:bg-white"
                                  >
                          ...
                          {/* Actions List */}
                          {(assessments[question.id]?.actions || []).length >
                            0 && (
                            <div className="mt-4 border-t border-gray-100 pt-4">
                              <h5 className="font-semibold text-gray-700 text-sm mb-3 flex items-center gap-2">
                                ...
                                Action Plan (
                                {
                                  (assessments[question.id]?.actions || [])
                                    .length
                                }
                                )
                              </h5>
                              <div className="space-y-2">
                                {(assessments[question.id]?.actions || []).map(
                                  (action: any) => (
                                    <div
                                      key={action.id}
      ...
      {isActionModalOpen && (
        <ActionModal
          ...
          subCategoryName={
            SIAMS_FRAMEWORK.flatMap((s) => s.inspectionQuestions)
              .find((q) => q.id === currentActionQuestionId)
