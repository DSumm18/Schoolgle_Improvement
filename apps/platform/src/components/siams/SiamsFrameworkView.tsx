...
import { SiamsStrandId, SiamsRating, SiamsQuestion, SiamsAssessment, STRAND_QUESTIONS, STRAND_INFO } from '@/lib/siams';
...
const RATING_OPTIONS: { value: SiamsRating; label: string; color: string }[] = [
    ...
    { value: 'ineffective', label: 'Ineffective', color: 'text-rose-700 bg-rose-50 border-rose-200' },
];
...
export default function SiamsFrameworkView({
    ...
    const [assessments, setAssessments] = useState<Record<string, SiamsAssessment>>({});
    ...
    const [selectedQuestion, setSelectedQuestion] = useState<SiamsQuestion | null>(null);
    ...
    const saveAssessment = async (rating: SiamsRating, notes: string) => {
        ...
        setSaving(true);
        try {
            const response = await fetch(`/api/siams/assessments/${selectedQuestion.id}`, {
                method: 'PATCH',
    ...
    const getRatingBadge = (rating: SiamsRating | null | undefined) => {
        ...
        return (
            <Badge className={`text-[10px] font-bold uppercase px-2 py-0.5 ${styles[rating]}`}>
    ...
    const allQuestions = useMemo(() => {
        return (Object.keys(STRAND_QUESTIONS) as SiamsStrandId[]).flatMap((strandId) =>
    ...
    return (
        <div className="space-y-4">
            ...
            {(Object.keys(STRAND_QUESTIONS) as SiamsStrandId[]).map((strandId) => {
                ...
                const strandAssessments = questions.filter((q) =>
                    assessments[q.id]?.school_rating && assessments[q.id].school_rating !== 'not_assessed'
                );
                const strandScore =
                                                ...
                                                >
                                                    <div className="flex items-start justify-between gap-4">
                                                        <div className="flex-1">
                                                            ...
                                                            {assessment?.self_assessment_notes && (
                                                                <div className="mt-2 p-2 bg-slate-50 rounded text-sm">
                                                                    <p className="text-xs text-slate-500 mb-1">
                                                                        Notes:
                                                                    </p>
                                                                    <p>{assessment.self_assessment_notes}</p>
                                                                </div>
                                                            )}
                                                        </div>
            ...
            <AssessmentDialog
                ...
                currentAssessment={selectedQuestion ? assessments[selectedQuestion.id] : undefined}
                saving={saving}
            />
        </div>
    );
...
function AssessmentDialog({
    ...
    const [selectedRating, setSelectedRating] = useState<SiamsRating | null>(null);
    const [notes, setNotes] = useState('');

    useEffect(() => {
        if (question) {
            setSelectedRating(currentAssessment?.school_rating || null);
            setNotes(currentAssessment?.self_assessment_notes || '');
        }
                ...
                <DialogHeader>
                    ...
                    {question && (
                        <DialogDescription className="text-left">
                            ...
                            <p className="font-medium text-slate-700 mt-2">{question.question}</p>
                        </DialogDescription>
                    )}
                </DialogHeader>
