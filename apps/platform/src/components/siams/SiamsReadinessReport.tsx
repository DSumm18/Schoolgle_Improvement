  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <ScoreCard
          title="Overall Score"
          score={readiness?.overall.overall_score || 0}
          rating={readiness?.overall.overall_rating}
        />
        <ScoreCard
          title="Strands Assessed"
          score={
            readiness?.overall?.strands?.filter(
              (s: SiamsStrandSummary) => s.average_score > 0,
            ).length || 0
          }
          total={readiness?.overall?.strands?.length || 7}
          isCount
        />
        <ScoreCard
          title="Evidence Linked"
          score={readiness?.overall.total_evidence || 0}
          isCount
        />
        <ScoreCard
          title="Action Items"
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {(readiness?.overall?.strands || []).map(
              (strand: SiamsStrandSummary, idx: number) => {
                const info = STRAND_INFO[strand.strand_id as SiamsStrandId];
                const gapCount = filteredGaps.filter(
                  (g: SiamsGapsAnalysis) => g.strand_id === strand.strand_id,
                ).length;
                      ...
                      <div className="flex-1">
                        ...
                        {gap.missing_evidence.length > 0 && (
                          <p className="text-xs text-slate-500 mt-1">
                            Missing: {gap.missing_evidence.join(", ")}
                          </p>
                        )}
                      </div>
      ...
      {/* Historical Trends */}
      {readiness?.snapshots && readiness.snapshots.length > 1 && (
        <Card>
          <CardHeader>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {readiness.snapshots
                    .slice(-6)
                    .map((snapshot: SiamsReadinessSnapshot) => (
                      <tr
                        key={snapshot.id}
                      ...
                      >
                        ...
                        {(
                          ...
                          const strandScore =
                            snapshot.strand_scores?.[strand] || 0;
                          return (
        ...
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-slate-500">
              {readiness?.overall?.strands?.every(
                (s: SiamsStrandSummary) => s.average_score > 0,
              )
