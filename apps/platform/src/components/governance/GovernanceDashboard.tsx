...
export default function GovernanceDashboard({ organizationId }: GovernanceDashboardProps) {
    const [statistics, setStatistics] = useState<GovernanceStatistics | null>(null);
    ...
    const fetchStatistics = async () => {
        try {
            const response = await fetch(`/api/governance/kpis?organizationId=${organizationId}`);
            if (response.ok) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 dark:text-white">Governance Portal</h1>
                    <p className="text-slate-500 mt-1">Manage governors, meetings, training, and compliance</p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
                {statCards.map((stat, idx) => {
                    ...
                    return (
                        ...
                        >
                            <Card className={`bg-gradient-to-br ${getStatCardStyles(stat.color)} border`}>
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                <TabsList className="bg-slate-100 dark:bg-slate-800 p-1 rounded-xl inline-flex h-12">
                    ...
                    <TabsTrigger value="governors" className="data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-lg px-6">
                        Governors
                    </TabsTrigger>
                    <TabsTrigger value="meetings" className="data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-lg px-6">
                        Meetings
                    </TabsTrigger>
                    <TabsTrigger value="training" className="data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-lg px-6">
                        Training
                    </TabsTrigger>
                    <TabsTrigger value="policies" className="data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-lg px-6">
                        Policies
                    </TabsTrigger>
                    <TabsTrigger value="visits" className="data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-lg px-6">
                        Visits
                    </TabsTrigger>
                    <TabsTrigger value="kpis" className="data-[state=active]:bg-white data-[state=active]:shadow-sm rounded-lg px-6">
                        KPIs
                    </TabsTrigger>
                </TabsList>

                <TabsContent value="overview" className="mt-6">
                    <OverviewTab organizationId={organizationId} statistics={statistics} />
                </TabsContent>

                <TabsContent value="governors" className="mt-6">
                    <GovernorsList organizationId={organizationId} onRefresh={fetchStatistics} />
                </TabsContent>

                <TabsContent value="meetings" className="mt-6">
                    <MeetingsCalendar organizationId={organizationId} onRefresh={fetchStatistics} />
                </TabsContent>

                <TabsContent value="training" className="mt-6">
                    <TrainingTracker organizationId={organizationId} onRefresh={fetchStatistics} />
                </TabsContent>

                <TabsContent value="policies" className="mt-6">
                    <PolicyReviewList organizationId={organizationId} onRefresh={fetchStatistics} />
                </TabsContent>

                <TabsContent value="visits" className="mt-6">
                    <VisitScheduler organizationId={organizationId} onRefresh={fetchStatistics} />
                </TabsContent>
                        ...
                        <div className="flex items-center justify-between p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500">{statistics?.vacant_positions} positions unfilled</p>
                                </div>
                            </div>
                    ...
                    {(statistics?.policies_overdue || 0) > 0 && (
                        <div className="flex items-center justify-between p-3 bg-rose-50 dark:bg-rose-900/20 rounded-lg">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-sm font-semibold">Overdue Policy Reviews</p>
                                    <p className="text-xs text-slate-500">{statistics?.policies_overdue} policies need attention</p>
                                </div>
                            </div>
                            <Button size="sm" variant="outline">Review</Button>
                        </div>
                        ...
                        <div className="flex items-center justify-between p-3 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500">{statistics?.expired_training_count} certificates expired</p>
                                </div>
                            </div>
                            <Button size="sm" variant="outline">Review</Button>
                        </div>
                    )}
                    {(!statistics?.vacant_positions && !statistics?.policies_overdue && !statistics?.expired_training_count) && (
                        <div className="flex items-center gap-3 p-3 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg">
                            <CheckCircle className="w-4 h-4 text-emerald-600" />
                            <p className="text-sm font-semibold text-emerald-700">All caught up! No urgent items.</p>
                        </div>
                    )}
                </CardContent>
            </Card>
