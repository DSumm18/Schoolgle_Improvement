...
export default function PolicyReviewList({
    ...
    const [selectedPolicy, setSelectedPolicy] = useState<GovernancePolicyReviewWithOwner | null>(null);
    ...
    const fetchPolicies = async () => {
        ...
        try {
            ...
            if (filterCategory !== 'all') params.append('policy_category', filterCategory);
    ...
    const handleMarkReviewed = async (policy: GovernancePolicyReviewWithOwner) => {
    ...
    const getCategoryBadge = (category: PolicyCategory) => {
        ...
        return (
            <Badge className={`text-[10px] font-normal uppercase ${styles[category]}`}>
                {category.replace('_', ' ')}
            </Badge>
        );
    };
    ...
    const getDueInfo = (policy: GovernancePolicyReviewWithOwner) => {
        ...
        return {
            text: nextReview.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }),
    ...
    const stats = useMemo(() => {
        return {
            ...
            under_review: policies.filter((p) => p.review_status === 'under_review').length,
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                <StatCard label="Total Policies" value={stats.total} icon={FileText} color="blue" />
                <StatCard label="Current" value={stats.current} icon={CheckCircle} color="emerald" />
                <StatCard label="Under Review" value={stats.under_review} icon={RefreshCw} color="blue" />
                <StatCard label="Overdue" value={stats.overdue} icon={AlertCircle} color="rose" />
                <StatCard label="Statutory" value={stats.statutory} icon={AlertTriangle} color="violet" />
            </div>
                                    ...
                                    return (
                                        <motion.tr
                                            ...
                                            className={`group hover:bg-slate-50 dark:hover:bg-slate-800/50 ${
                                                policy.days_overdue > 0 ? 'bg-rose-50/50 dark:bg-rose-900/10' : ''
                                            }`}
                                        >
                                            <TableCell>
                                                <div className="flex items-center gap-2">
                                                    {policy.is_statutory && (
                                                        <AlertTriangle className="w-3.5 h-3.5 text-violet-500" title="Statutory" />
                                                    )}
                                                    <span className="font-medium text-sm">{policy.policy_name}</span>
                                                </div>
                                            </TableCell>
                                            <TableCell>{getCategoryBadge(policy.policy_category)}</TableCell>
                                            <TableCell>
                                                {policy.policy_owner_name ? (
                                                    <div className="flex items-center gap-2">
                                                        <User className="w-3.5 h-3.5 text-slate-400" />
                                                        <span className="text-sm">{policy.policy_owner_name}</span>
                                                    </div>
                                                ) : (
                                            ...
                                            <TableCell>
                                                <div className={`flex items-center gap-1.5 text-sm ${dueInfo.color}`}>
                                            ...
                                            <TableCell>{getStatusBadge(policy.review_status)}</TableCell>
                                            <TableCell className="text-right">
                                                <div className="flex items-center justify-end gap-1">
                                                    {policy.days_overdue <= 0 && policy.review_status !== 'under_review' && (
                                                        <Button
                                                            variant="ghost"
                                                            size="sm"
