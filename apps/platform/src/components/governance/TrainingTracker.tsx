...
import { GovernorTraining, TrainingType } from "@/lib/governance";
...
export default function TrainingTracker({
  ...
  const filteredTraining = useMemo(() => {
    return enrichedTraining.filter((record) => {
      ...
      const matchesType =
        filterType === "all" || record.training_type === filterType;
  ...
  const stats = useMemo(() => {
    ...
    const valid = enrichedTraining.filter(
      (t) => t.status.status === "valid",
    ).length;
    const expiring = enrichedTraining.filter(
      (t) => t.status.status === "expiring",
    ).length;
    const expired = enrichedTraining.filter(
      (t) => t.status.status === "expired",
    ).length;
              ...
              {loading ? (
                ...
                filteredTraining.map((record) => (
                  ...
                  >
                    ...
                    <TableCell>
                      {getTrainingTypeBadge(record.training_type)}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Badge
                        className={`text-[10px] font-bold uppercase px-2 py-0.5 ${record.status.color}`}
                      >
                        {record.status.label}
                      </Badge>
                    </TableCell>
