...
export default function KPIsDashboard({ organizationId }: KPIsDashboardProps) {
  ...
  const fetchKPIs = async () => {
    ...
    try {
      ...
      toast.error("Failed to load governance KPIs");
    } finally {
      setLoading(false);
    }
  };
  ...
  const createSnapshot = async () => {
    try {
      ...
      toast.error("Failed to create KPI snapshot");
    }
  };
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Track key governance metrics over time
          </p>
        </div>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-xs text-slate-500">
                        {skill.governors.length} governor
                        {skill.governors.length !== 1 ? "s" : ""}
                      </span>
                    </div>
                ...
                <tbody>
                  {kpiData.historical.slice(-6).map((snapshot) => (
                    <tr key={snapshot.id} className="border-b border-slate-100">
                      ...
                      <td className="text-right py-2 px-3">
                        {snapshot.policies_current}/
                        {snapshot.policies_outstanding_review}
                      </td>
                    </tr>
                  ))}
                </tbody>
