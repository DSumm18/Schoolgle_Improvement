...
export default function MeetingsCalendar({
    ...
    const [selectedMeeting, setSelectedMeeting] = useState<GovernorMeeting | null>(null);
    ...
    const fetchMeetings = async () => {
        ...
        try {
            ...
            if (filterCommittee !== 'all') params.append('meeting_type', filterCommittee);
    ...
    const getCommitteeBadge = (meetingType: CommitteeType, committee: string | null) => {
        ...
        return (
            <Badge className={`text-[10px] font-normal uppercase ${colors[meetingType]}`}>
                {label}
            </Badge>
        );
    };
                ...
                <CardContent className="p-4">
                    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                        <div className="flex flex-wrap items-center gap-3">
                            ...
                            <Select value={filterCommittee} onValueChange={(value) => setFilterCommittee(value as any)}>
                        ...
                        <Card>
                            <CardContent className="p-6">
                                ...
                                <p className="text-sm text-slate-500 text-center py-8">
                                    Calendar view coming soon. Use list view for full functionality.
                                </p>
                            </CardContent>
                        </Card>
                    )}
...
interface MeetingCardProps {
    ...
    getCommitteeBadge: (type: CommitteeType, committee: string | null) => React.ReactNode;
}
    ...
    return (
        <motion.div
            ...
            className={`bg-white dark:bg-slate-900 rounded-xl border p-4 hover:shadow-md transition-shadow ${
                isPast ? 'border-slate-200 dark:border-slate-700' : 'border-blue-200 dark:border-blue-800'
            }`}
        >
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-4 mt-2 text-sm text-slate-600">
                        <div className="flex items-center gap-1.5">
                            ...
                            <span className="font-medium">{formatMeetingDate(meeting.scheduled_date)}</span>
                        </div>

                        {meeting.scheduled_time && (
                            <div className="flex items-center gap-1.5">
                                <Clock className="w-4 h-4 text-slate-400" />
                                <span>{formatMeetingTime(meeting.scheduled_time, meeting.duration_minutes)}</span>
                            </div>
                        )}
                    ...
                    {/* Attendance */}
                    {meeting.invited_governors && meeting.invited_governors.length > 0 && (
                        <div className="mt-3 flex items-center gap-2">
                            <Users className="w-3.5 h-3.5 text-slate-400" />
                            <span className="text-xs text-slate-500">
                                {meeting.attended_governors?.length || 0}/{meeting.invited_governors.length} attending
                            </span>
                        </div>
                    )}
                </div>
