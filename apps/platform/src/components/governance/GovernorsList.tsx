...
export default function GovernorsList({
    ...
    const [filterStatus, setFilterStatus] = useState<GovernorStatus | 'all'>('all');
    ...
    const getRoleBadge = (role: GovernorRole) => {
        ...
        return (
            <Badge className={`text-[10px] font-bold uppercase px-2 py-0.5 ${styles[role] || ''}`}>
                {role.replace('_', ' ')}
            </Badge>
        );
    };
    ...
    const calculateAttendance = (governor: Governor) => {
        ...
        const percentage = Math.round((governor.meetings_attended / governor.meetings_total) * 100);
        const color = percentage >= 80 ? 'text-emerald-600' : percentage >= 60 ? 'text-amber-600' : 'text-rose-600';
        return (
            <span className={`font-bold ${color}`}>
                {percentage}%
                <span className="text-slate-400 font-normal"> ({governor.meetings_attended}/{governor.meetings_total})</span>
            </span>
        );
    };

    const getTermStatus = (governor: Governor) => {
        if (!governor.end_date) return <span className="text-slate-400">No end date</span>;
        ...
        if (daysUntil < 90) {
            return (
                <span className="text-amber-600 font-bold">
                    Expires {endDate.toLocaleDateString('en-GB', { month: 'short', day: 'numeric' })}
                </span>
            );
        }
        return (
            <span className="text-slate-600">
                Until {endDate.toLocaleDateString('en-GB', { month: 'short', year: '2-digit' })}
            </span>
        );
    };

    const committeeBadges = (committees: CommitteeName[]) => {
        if (!committees || committees.length === 0) return <span className="text-slate-400 text-sm">None</span>;

        return (
            <div className="flex flex-wrap gap-1">
                {committees.slice(0, 2).map((committee) => (
                    <Badge key={committee} variant="outline" className="text-[9px] font-normal">
                        {committee}
                    </Badge>
                ))}
                {committees.length > 2 && (
                            ...
                            {loading ? (
                                ...
                                <TableRow>
                                    <TableCell colSpan={8} className="text-center py-12">
                                        <div className="flex flex-col items-center gap-3">
                                            ...
                                            <p className="text-slate-500 font-semibold">No governors found</p>
                                            <Button
                                                variant="outline"
                                                size="sm"
                                                onClick={() => {
                                ...
                                filteredGovernors.map((governor, idx) => (
                                    ...
                                    >
                                        ...
                                        <TableCell>{getTypeBadge(governor.governor_type)}</TableCell>
                                        <TableCell>
                                            {governor.role ? (
                                        ...
                                        <TableCell>{committeeBadges(governor.committee_assignment)}</TableCell>
