...
export default function SchoolDetailView({
  ...
  const totalAssessments = school.categories.reduce(
  ...
  const getCategoryIcon = (name: string) => {
    ...
    if (n.includes("safety") || n.includes("health"))
      return <ShieldCheck className="h-4 w-4" />;
    if (n.includes("energy") || n.includes("efficiency"))
      return <Zap className="h-4 w-4" />;
    if (n.includes("accessibility"))
    ...
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
      ...
      {onBack && (
        <Button
          variant="ghost"
          onClick={onBack}
          className="group text-slate-500 hover:text-teal-600 p-0"
        >
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
          ...
          <div className="flex items-center gap-8 bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-6 border border-slate-100 dark:border-slate-800">
            <div className="text-center">
              <div
                className={`text-4xl font-black ${averageScore >= 80 ? "text-emerald-600" : "text-teal-600"}`}
              >
                {school.overallScore}%
              </div>
            ...
            <div className="space-y-4">
              <div className="flex gap-2">
                <Button
                  size="sm"
                  className="bg-teal-600 hover:bg-teal-700 h-9 font-bold text-[10px] uppercase tracking-wider"
                >
        ...
        <TabsList className="bg-slate-100/80 p-1 rounded-xl h-auto gap-1">
          {[
            ...
            <TabsTrigger
              key={tab.value}
              value={tab.value}
              className="px-6 py-2.5 rounded-lg text-xs font-bold uppercase tracking-widest data-[state=active]:bg-white data-[state=active]:text-teal-600 data-[state=active]:shadow-sm transition-all"
            >
              {tab.label}
            </TabsTrigger>
          ))}
              ...
              <CardContent className="space-y-6">
                {school.categories.map((category) => (
                  <div
                    ...
                    className="space-y-2 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100/50 transition-colors hover:border-teal-100"
                  >
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <Badge
                        variant="outline"
                        className={`font-black border-0 ${category.average >= 80 ? "bg-emerald-500/10 text-emerald-600" : "bg-teal-500/10 text-teal-600"}`}
                      >
                        {category.average}%
                      </Badge>
                    </div>
              ...
              <Card className="border-0 shadow-lg shadow-slate-200/50 overflow-hidden">
                <CardHeader className="bg-teal-600 text-white">
                  ...
                  <CardDescription className="text-teal-100/80">
                    Highest scoring areas of the estate
                  </CardDescription>
                </CardHeader>
              ...
              <Card className="border-0 shadow-lg shadow-slate-200/50 overflow-hidden">
                <CardHeader className="bg-rose-600 text-white">
                  ...
                  <CardDescription className="text-rose-100/80">
                    Urgent maintenance or compliance issues
                  </CardDescription>
                </CardHeader>
                <CardContent className="p-0">
                  <div className="divide-y divide-slate-100 dark:divide-slate-800">
        ...
        >
          <div className="grid grid-cols-1 gap-8">
            {school.categories.map((category) => (
              <Card
                key={category.categoryName}
                className="border-0 shadow-lg shadow-slate-200/50 overflow-hidden"
              >
                <CardHeader className="border-b border-slate-100 dark:border-slate-800 bg-slate-50/50">
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-3">
                      <div className="bg-white p-2 rounded-xl shadow-sm border border-slate-100 text-teal-600">
                        {getCategoryIcon(category.categoryName)}
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-3">
                      <div className="text-right">
                        ...
                        <div
                          className={`text-lg font-black ${category.average >= 80 ? "text-emerald-600" : "text-teal-600"}`}
                        >
                          {category.average}%
                        </div>
                      </div>
                ...
                <CardContent className="p-0">
                  <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-100 dark:divide-slate-800">
                    {category.assessments.map((assessment, idx) => (
                      <div
                        key={idx}
                        className="flex flex-col p-6 hover:bg-slate-50/50 transition-colors"
                      >
          ...
          <Card className="border-0 shadow-lg shadow-slate-200/50">
            <CardHeader>
              <CardTitle className="text-xl font-bold">
                Maintenance & Compliance Backlog
              </CardTitle>
              <CardDescription className="text-sm font-medium">
                Critical items requiring immediate management attention
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {school.categories
                .flatMap((cat) =>
                  cat.assessments.map((a) => ({ ...a, cat: cat.categoryName })),
                )
                .filter((a) => a.score < 75)
                .sort((a, b) => a.score - b.score)
                .map((item, idx) => (
                  <div
                    key={idx}
                    className="group flex items-center justify-between p-5 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-transparent hover:border-rose-100 hover:bg-rose-50/30 transition-all"
                  >
                    <div className="flex gap-4 items-center">
                      <div
                        className={`p-3 rounded-xl ${item.score < 50 ? "bg-rose-100 text-rose-600" : "bg-amber-100 text-amber-600"}`}
                      >
                        <AlertCircle className="h-5 w-5" />
                      </div>
                      <div>
                        <div className="text-sm font-bold text-slate-900 dark:text-slate-100 group-hover:text-rose-900">
                          {item.name}
                        </div>
                    ...
                    <div className="flex items-center gap-6">
                      <div className="hidden sm:block text-right">
                        ...
                        <div
                          className={`text-xs font-black uppercase ${item.score < 50 ? "text-rose-600" : "text-amber-600"}`}
                        >
                          {item.score < 30
                            ? "Critical"
                            : item.score < 50
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        className="text-[10px] font-bold uppercase tracking-wider h-8 border-slate-200 hover:bg-white"
                      >
                        Assign Task
                      </Button>
                    </div>
