...
import { CheckCircle2, Clock, AlertCircle, ArrowRight, Calendar, User, Building2, BookOpen, Wrench, Shield, GraduationCap, MoreHorizontal, Filter } from "lucide-react";
...
const ROLE_MODULES: Record<Role, string[]> = {
    admin: ['governance', 'improvement', 'teaching-learning', 'estates', 'compliance', 'finance', 'hr', 'send'],
    headteacher: ['governance', 'improvement', 'teaching-learning', 'estates', 'compliance', 'hr'],
...
const MODULE_COLORS: Record<string, { bg: string; text: string; border: string }> = {
    governance: { bg: 'bg-purple-50 dark:bg-purple-900/20', text: 'text-purple-600 dark:text-purple-400', border: 'border-purple-200 dark:border-purple-800' },
    improvement: { bg: 'bg-blue-50 dark:bg-blue-900/20', text: 'text-blue-600 dark:text-blue-400', border: 'border-blue-200 dark:border-blue-800' },
    'teaching-learning': { bg: 'bg-emerald-50 dark:bg-emerald-900/20', text: 'text-emerald-600 dark:text-emerald-400', border: 'border-emerald-200 dark:border-emerald-800' },
    estates: { bg: 'bg-amber-50 dark:bg-amber-900/20', text: 'text-amber-600 dark:text-amber-400', border: 'border-amber-200 dark:border-amber-800' },
    compliance: { bg: 'bg-rose-50 dark:bg-rose-900/20', text: 'text-rose-600 dark:text-rose-400', border: 'border-rose-200 dark:border-rose-800' },
    finance: { bg: 'bg-green-50 dark:bg-green-900/20', text: 'text-green-600 dark:text-green-400', border: 'border-green-200 dark:border-green-800' },
    hr: { bg: 'bg-indigo-50 dark:bg-indigo-900/20', text: 'text-indigo-600 dark:text-indigo-400', border: 'border-indigo-200 dark:border-indigo-800' },
    send: { bg: 'bg-pink-50 dark:bg-pink-900/20', text: 'text-pink-600 dark:text-pink-400', border: 'border-pink-200 dark:border-pink-800' },
};

type FilterStatus = 'all' | 'overdue' | 'due_today' | 'due_this_week' | 'completed';
...
export function MyTasksWidget({ limit = 5 }: MyTasksWidgetProps) {
    ...
    // Fetch user's tasks
    const { data: tasksData, isLoading, error } = useSWR(
        organization?.id && user?.id
            ? [`/api/tasks?organizationId=${organization.id}&assigneeId=${user.id}&limit=50`, organization.id, user.id]
            : null,
        async ([url]) => {
            try {
    ...
    const filteredTasks = useMemo(() => {
        ...
        switch (statusFilter) {
            case 'overdue':
                return tasks.filter(t => t.due_date && t.due_date < today && t.status !== 'completed');
            case 'due_today':
    ...
    // Task statistics
    const stats = useMemo(() => {
        if (!tasksData?.tasks) return { total: 0, overdue: 0, dueToday: 0, dueThisWeek: 0 };
        ...
        return {
            total: userTasks.filter((t: UnifiedTask) => t.status !== 'completed').length,
            overdue: userTasks.filter((t: UnifiedTask) =>
                t.due_date && t.due_date < today && t.status !== 'completed'
            ).length,
            dueToday: userTasks.filter((t: UnifiedTask) =>
                t.due_date === today && t.status !== 'completed'
            ).length,
            dueThisWeek: userTasks.filter((t: UnifiedTask) =>
                t.due_date &&
                t.due_date >= today &&
                t.due_date <= weekFromNowStr &&
                t.status !== 'completed'
    ...
    const getModuleInfo = (moduleName: string) => {
        return {
            ...
            name: moduleName === 'general' ? 'General Tasks' :
        ...
        return (
            <div className="glass-card rounded-2xl p-8 border border-slate-200/50 dark:border-slate-800/50">
                <div className="text-center py-8">
                    ...
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Unable to load tasks</h3>
            ...
            <div className="px-8 py-6 bg-slate-50/50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800/50">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white">My Tasks</h2>
                            <p className="text-xs text-slate-500 font-medium tracking-wide uppercase">
                                {allowedModules.length > 1 ? 'Across all modules' : getModuleInfo(allowedModules[0]).name}
                            </p>
                        </div>
                    </div>
                ...
                <div className="flex gap-2 mt-4 overflow-x-auto pb-1">
                    {(['all', 'overdue', 'due_today', 'due_this_week'] as FilterStatus[]).map(filter => {
                        const count = filter === 'all' ? stats.total :
                            filter === 'overdue' ? stats.overdue :
                            filter === 'due_today' ? stats.dueToday :
                            stats.dueThisWeek;
                        return (
                            <button
                                key={filter}
                    ...
                    {filteredTasks.length === 0 ? (
                        ...
                        >
                            ...
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white">
                                {statusFilter === 'all' ? 'No pending tasks' : `No ${statusFilter.replace('_', ' ')} tasks`}
                            </h3>
                                ...
                                >
                                    ...
                                    >
                                        <div className="flex items-start gap-3">
                                            {/* Module Icon */}
                                            <div className={`p-2 rounded-lg ${moduleInfo.colors.bg} ${moduleInfo.colors.text} shrink-0`}>
                                                <ModuleIcon size={16} />
                                            </div>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center justify-between gap-2 mb-1">
                                                    <h3 className={`font-semibold text-sm truncate ${overdue ? 'text-rose-700 dark:text-rose-300' : 'text-slate-900 dark:text-white'}`}>
                                                        {task.title}
                                                    </h3>
                                                    <div className="flex items-center gap-2 shrink-0">
                                                        ...
                                                        <MoreHorizontal size={14} className="text-slate-400 group-hover:text-blue-500 transition-colors" />
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-wider text-slate-400">
                                                    ...
                                                    <span className={`px-2 py-0.5 rounded ${moduleInfo.colors.bg} ${moduleInfo.colors.text}`}>
                                                        {moduleInfo.name}
                                                    </span>
                                                </div>
                                            </div>
