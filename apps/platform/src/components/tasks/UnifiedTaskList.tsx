...
interface UnifiedTaskListProps {
  ...
  onRefresh?: () => void;
  onAddTask?: () => void;
  onSelectTask?: (task: UnifiedTask | null) => void;
}

export type { UnifiedTaskListProps };
...
export default function UnifiedTaskList({
  ...
  const fetchTasks = async () => {
    ...
    try {
      ...
      if (filters.assignee_id)
        params.append("assigneeId", filters.assignee_id.join(","));
      if (filters.team_id) params.append("teamId", filters.team_id.join(","));
  ...
  const kanbanGroups = useMemo(() => {
    const groups: Partial<Record<TaskStatus, UnifiedTask[]>> = {
      not_started: [],
      in_progress: [],
      review: [],
      completed: [],
  ...
  const getStatusBadge = (status: TaskStatus) => {
    ...
    return (
      <Badge
        className={`text-[10px] font-bold uppercase px-2 py-0.5 ${option?.color}`}
      >
        {option?.label}
      </Badge>
    );
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setFilters({
                    ...filters,
                    task_type:
                      value === "all" ? undefined : [value as TaskType],
                  })
                }
              >
                <SelectTrigger className="w-36">
      ...
      {summary && (
        <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
          ...
          <StatCard
            label="Active"
            value={
              (summary.total_tasks || 0) - (summary.by_status?.completed || 0)
            }
          />
          <StatCard
            label="Completed"
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b bg-muted/50">
              ...
              <th className="text-right py-3 px-4 text-xs font-black uppercase text-slate-500">
                Actions
              </th>
            </tr>
          </thead>
          <tbody>
            {tasks.map((task) => (
              ...
              >
                  ...
                  {task.assignee_id ? (
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-[10px] font-bold text-white">
                        {(
                          (task as UnifiedTaskWithAssignee).assignee_name || "U"
                        )
                          .charAt(0)
                          .toUpperCase()}
                      </div>
                ...
                <td className="py-3 px-4">
                  <div
                    className={`flex items-center gap-1.5 text-xs ${isOverdue(task) ? "text-rose-600" : "text-slate-600"}`}
                  >
...
interface KanbanBoardProps {
  groups: Partial<Record<TaskStatus, UnifiedTask[]>>;
  loading: boolean;
  onEdit: (task: UnifiedTask) => void;
  onToggleComplete: (task: UnifiedTask) => void;
  getStatusBadge: (status: TaskStatus) => React.ReactNode;
...
function KanbanCard({
  ...
  return (
    ...
    >
      ...
      <h4
        className={`text-sm font-medium mb-2 ${isCompleted ? "line-through text-slate-400" : ""}`}
      >
        {task.title}
      </h4>
