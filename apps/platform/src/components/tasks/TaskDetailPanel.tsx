...
import { UnifiedTask, TaskComment, TaskTimeEntry } from '@/lib/tasks';
...
export default function TaskDetailPanel({
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'comments' | 'time'>('overview');
    ...
    const fetchTaskData = async () => {
        ...
        try {
            const [commentsRes, timeRes] = await Promise.all([
                fetch(`/api/tasks/${task.id}/comments?organizationId=${organizationId}`),
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 overflow-y-auto">
                ...
                {activeTab === 'overview' && (
                    <div className="p-4 space-y-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                            <Badge className={`text-[10px] font-bold uppercase px-2 py-1 ${getStatusColor(task.status)}`}>
                                {task.status.replace('_', ' ')}
                            </Badge>
                            <Badge className={`text-[10px] font-bold uppercase px-2 py-1 ${getPriorityColor(task.priority)}`}>
                                {task.priority}
                            </Badge>
                            ...
                            <div className="grid grid-cols-2 gap-4 mt-3 text-sm">
                                ...
                                {task.assignee_name && (
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-slate-600">{task.assignee_name}</span>
                                    </div>
                                )}
                                {task.estimated_hours && (
                                    <div className="flex items-center gap-2">
                                        <Timer className="w-4 h-4 text-slate-400" />
                                        <span className="text-slate-600">{task.estimated_hours}h estimated</span>
                                    </div>
                                )}
                            </div>
                        </Section>
                ...
                {activeTab === 'comments' && (
                    <div className="p-4">
                        <CommentList comments={comments} onAddComment={addComment} loading={loading} />
                    </div>
                )}
...
function TimeTracking({ timeEntries, totalHours, onAddTime, loading }: TimeTrackingProps) {
                    ...
                    <div key={entry.id} className="flex items-center justify-between p-2 border rounded-lg">
                        <div>
                            ...
                            <p className="text-xs text-slate-500">
                                {entry.minutes}min • {entry.date ? format(new Date(entry.date), 'd MMM') : ''}
                            </p>
                        </div>
