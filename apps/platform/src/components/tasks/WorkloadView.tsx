...
export default function WorkloadView({ organizationId }: WorkloadViewProps) {
    const [workloadData, setWorkloadData] = useState<GetTeamWorkloadResponse | null>(null);
    ...
    const fetchWorkload = async () => {
        setLoading(true);
        try {
            const response = await fetch(`/api/teams/workload?organizationId=${organizationId}`);
            if (response.ok) {
                                    ...
                                    >
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                {user.team_name && (
                                                    <p className="text-xs text-slate-500">{user.team_name}</p>
                                                )}
                                            </div>

                                            {/* Task Count */}
                                            <div className="text-center">
                                                <p className="text-2xl font-bold">{user.total_tasks || 0}</p>
                                                <p className="text-[10px] uppercase text-slate-500">Tasks</p>
                                            </div>
