...
export default function SDPBuilder({
  initialPriorities,
  onSave,
}: SDPBuilderProps) {
  const [priorities, setPriorities] =
    useState<SDPPriorityData[]>(initialPriorities);
  ...
  const updatePriority = (id: string, updates: Partial<SDPPriorityData>) => {
    setPriorities(
      priorities.map((p) => (p.id === id ? { ...p, ...updates } : p)),
    );
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center bg-white dark:bg-slate-900/50 p-8 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-xl">
        <div>
          <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">
            Strategic Priorities
          </h3>
          <p className="text-slate-500 font-bold text-sm">
            Define and track your high-level school improvement goals
          </p>
        </div>
        <button
          onClick={addPriority}
      ...
      <div className="space-y-4">
        {priorities.map((p) => (
          <motion.div
            key={p.id}
            layout
            className={`bg-white dark:bg-slate-900/50 rounded-2xl border transition-all ${expandedPriority === p.id ? "border-blue-500 ring-4 ring-blue-50 shadow-lg" : "border-slate-100 dark:border-slate-800 shadow-lg"}`}
          >
            <div
              onClick={() =>
                setExpandedPriority(expandedPriority === p.id ? null : p.id)
              }
              className="p-8 flex items-center justify-between cursor-pointer group"
            >
              <div className="flex items-center gap-6">
                <div
                  className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-xl transition-all ${expandedPriority === p.id ? "bg-blue-600 text-white shadow-xl rotate-3" : "bg-slate-50 dark:bg-slate-800 text-slate-400 group-hover:text-blue-500"}`}
                >
                  {p.number}
                </div>
                <div>
                  <h4 className="text-xl font-black text-slate-900 dark:text-white group-hover:text-blue-600 transition-colors">
                    {p.title || "Untitled Priority"}
                  </h4>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-3">
                        ...
                        <input
                          value={p.title}
                          onChange={(e) =>
                            updatePriority(p.id, { title: e.target.value })
                          }
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                      ...
                      <div className="md:col-span-2 space-y-3">
                        ...
                        <textarea
                          value={p.rationale}
                          onChange={(e) =>
                            updatePriority(p.id, { rationale: e.target.value })
                          }
                          rows={1}
                      ...
                      <div className="space-y-4">
                        {p.milestones.map((m, mIdx) => (
                          <div key={m.id} className="flex gap-4 items-center">
                            ...
                            <button
                              onClick={() => {
                                const newMs = p.milestones.filter(
                                  (x) => x.id !== m.id,
                                );
                                updatePriority(p.id, { milestones: newMs });
                              }}
      ...
      <div className="flex justify-end pt-10">
        <button
          ...
          className="flex items-center gap-3 px-12 py-5 bg-gradient-to-r from-sky-600 to-sky-700 text-white rounded-xl font-black text-sm uppercase tracking-widest shadow-lg hover:scale-105 active:scale-95 transition-all group"
        >
