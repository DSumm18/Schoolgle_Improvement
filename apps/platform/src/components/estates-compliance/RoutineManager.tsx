...
export function RoutineManager() {
  ...
  const [editingRoutine, setEditingRoutine] = useState<Partial<Routine> | null>(
    null,
  );
  ...
  const handleSaveRoutine = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(
          editingRoutine.id ? "Routine updated" : "Routine created",
        );
        setDialogOpen(false);
        fetchRoutines();
      } else {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Configure custom daily or weekly routines for your site staff
          </p>
        </div>
        <Button
          onClick={() => {
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {routines.map((routine) => (
          <Card
            key={routine.id}
            className="relative transition-all hover:shadow-lg border-2 border-primary/5 hover:border-primary/20 overflow-hidden group"
          >
            <div className="absolute top-0 right-0 p-2 opacity-0 group-hover:opacity-100 transition-opacity">
              <Badge
                variant="outline"
                className="bg-white/80 backdrop-blur-sm text-[10px] uppercase tracking-wider font-bold"
              >
                {routine.type}
              </Badge>
            </div>
            <CardHeader className="pb-3">
              <div className="flex items-start justify-between">
                ...
                {!routine.is_active && (
                  <Badge
                    variant="secondary"
                    className="bg-gray-100 text-gray-400 text-[10px] py-0 px-1.5 h-5"
                  >
                    Inactive
                  </Badge>
                )}
              ...
              <div className="flex items-center justify-between pt-4 border-t border-gray-50">
                <div className="flex -space-x-1.5">
                  {routine.items?.slice(0, 4).map((item, i) => (
                    <div
                      key={i}
                      className="w-6 h-6 rounded-full bg-white border border-gray-100 flex items-center justify-center text-[10px] shadow-sm"
                    >
                      {item.icon || "📋"}
                    </div>
                  ))}
      ...
      {routines.length === 0 && (
        <div className="text-center py-20 bg-gradient-to-br from-gray-50/50 to-white rounded-3xl border-2 border-dashed border-gray-200">
          ...
          <p className="text-sm text-muted-foreground mb-6 max-w-sm mx-auto">
            Create bespoke daily or weekly checklists tailored to your school's
            specific requirements.
          </p>
          <Button
            onClick={() => setDialogOpen(true)}
            className="px-6 rounded-full font-bold shadow-md shadow-primary/20"
          >
            Get Started
          </Button>
        </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    setEditingRoutine({ ...editingRoutine, type: v as any })
                  }
                >
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-bold flex items-center gap-2">
                  ...
                  <Badge
                    variant="secondary"
                    className="text-[10px] h-4 px-1.5 bg-gray-100 text-gray-500 border-none"
                  >
                ...
                <Button
                  ...
                  className="h-8 rounded-full border-primary/30 text-primary hover:bg-primary/5 px-4 font-bold text-xs"
                >
              ...
              <div className="space-y-4">
                {editingRoutine?.items?.map((item, idx) => (
                  <div
                    key={idx}
                    className="p-5 rounded-2xl border border-gray-100 bg-white shadow-sm relative group hover:border-primary/20 transition-colors"
                  >
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-gray-50">
                      <div className="flex items-center gap-4">
                        ...
                        <label className="flex items-center gap-2 text-[10px] font-bold text-gray-400 cursor-pointer select-none">
                          <input
                            ...
                            onChange={(e) =>
                              updateItem(idx, "requiresPhoto", e.target.checked)
                            }
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Badge
                          variant="outline"
                          className="text-[8px] font-bold uppercase tracking-tight py-0 px-1 border-gray-200 text-gray-400"
                        >
                          Step {idx + 1}
                        </Badge>
                ...
                {(!editingRoutine?.items ||
                  ...
                  <div className="py-12 bg-gray-50/50 rounded-3xl border-2 border-dashed border-gray-100 flex flex-col items-center justify-center text-center px-4">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Add items to this routine to create a completion
                      checklist.
                    </p>
                    <Button
                      variant="ghost"
          ...
          <div className="p-6 border-t bg-white flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button
                ...
                className="font-bold text-xs h-10 px-6 rounded-xl hover:bg-gray-50"
              >
                Cancel
              </Button>
              <Button
