...
import { Finding, FindingClassification } from '@/lib/estates-compliance/findings-database';
import { getClassificationBadgeClasses, formatClassification } from '@/lib/estates-compliance/findings-database';
...
const severityConfig: Record<Finding['severity'], { label: string; className: string }> = {
...
const classificationConfig: Record<FindingClassification, { label: string; description: string; icon: string }> = {
  statutory: {
...
export function FindingsList({
  ...
  const counts: Record<FindingClassification | 'all', number> = {
    ...
    good_practice: findings.filter(f => f.classification === 'good_practice').length,
    contractor_suggestion: findings.filter(f => f.classification === 'contractor_suggestion').length,
  };
...
function FindingCard({ finding, isExpanded, onToggle, onDecisionClick, showDecisionButtons }: FindingCardProps) {
  const config = classificationConfig[finding.classification];
  const severity = severityConfig[finding.severity];

  return (
    <div className={`border rounded-lg overflow-hidden transition-all ${
      finding.classification === 'statutory' ? 'border-red-200 bg-red-50/30' :
      finding.classification === 'good_practice' ? 'border-amber-200 bg-amber-50/30' :
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className={getClassificationBadgeClasses(finding.classification)}>
                {config.label}
              </span>
              {severity && (
                <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${severity.className}`}>
                  {severity.label}
                </span>
              )}
            </div>
            ...
            {finding.confidence !== undefined && (
              <div className="text-xs text-muted-foreground">
                Classification confidence: {Math.round(finding.confidence * 100)}%
                {finding.confidence < 0.7 && (
                  <span className="ml-2 text-amber-600">(Manual review recommended)</span>
                )}
              </div>
            )}
          </div>
        ...
        {isExpanded && (
          <div className="mt-4 pt-4 border-t space-y-3">
            ...
            {finding.explanation && (
              <div>
                <h4 className="text-sm font-medium mb-1">Why this classification:</h4>
...
function DecisionSupportModal({ finding, onClose, onApprove, onDecline, onDefer }: DecisionSupportModalProps) {
  const config = classificationConfig[finding.classification];
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <h3 className="text-lg font-semibold mt-2">{finding.description}</h3>
            {finding.estimatedCost && (
              ...
              {finding.classification === 'good_practice' && (
                <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg">
                  ...
                  <p className="text-sm text-amber-800 mb-3">
                    This is recommended by industry guidance but is not legally required:
                  </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t">
            ...
            {finding.classification !== 'statutory' && (
              <button
                onClick={() => onDefer(new Date(Date.now() + 90 * 24 * 60 * 60 * 1000))}
