...
export const AssessmentModal: React.FC<AssessmentModalProps> = ({
  ...
  const [rating, setRating] = useState(
    initialData.schoolRating || "not_assessed",
  );
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
        ...
        >
          ...
          <div className="p-8 space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                {RATINGS.map((r) => (
                  <button
                    ...
                    className={`px-4 py-3 rounded-2xl border-2 text-[10px] font-black uppercase tracking-wider transition-all flex flex-col items-center justify-center gap-2 ${
                      rating === r.id
                  ...
                  >
                    <div
                      className={`w-2 h-2 rounded-full ${rating === r.id ? `bg-${r.color}-500 animate-pulse` : "bg-slate-200"}`}
                    />
                    {r.label}
                  </button>
                ))}
            ...
            <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-800/50 rounded-2xl p-4 flex gap-3 items-start">
              ...
              <p className="text-[11px] text-amber-800 dark:text-amber-400 font-medium">
                Ratings should be evidence-based. If your AI Evidence Score is
                significantly lower than your self-rating, ensure all relevant
          ...
          <div className="px-8 py-6 bg-slate-50 dark:bg-slate-800/30 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
            ...
            <button
              onClick={() =>
                onSave(subcategory.id, {
                  ...initialData,
                  schoolRating: rating,
                  schoolRationale: rationale,
                })
              }
