...
export const SubcategoryAssessment: React.FC<SubcategoryAssessmentProps> = ({
  ...
  const ratingStyle = assessment.schoolRating
    ? RATING_STYLES[assessment.schoolRating]
    : null;
  const aiRatingStyle = assessment.aiRating
    ? RATING_STYLES[assessment.aiRating]
    : null;
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="p-4 bg-white dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm">
              ...
              {assessment.schoolRating ? (
                <div className="flex items-center gap-2">
                  <div
                    className={`px-3 py-1 rounded-full text-xs font-bold border ${ratingStyle?.bg}`}
                  >
            ...
            <div className="p-4 bg-white dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 shadow-sm relative overflow-hidden group">
              ...
              {assessment.aiRating ? (
                <div className="flex items-center gap-2">
                  <div
                    className={`px-3 py-1 rounded-full text-xs font-bold border ${aiRatingStyle?.bg}`}
                  >
        ...
        <div className="w-full lg:w-80 flex flex-col gap-4">
          ...
          <div className="space-y-2">
            {subcategory.evidenceRequired.map((item) => {
              const matches =
                evidenceMatches[`${subcategory.id}_${item.id}`] || [];
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3 min-w-0">
                      <div
                        className={`p-2 rounded-lg ${hasEvidence ? "bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600" : "bg-slate-200/50 dark:bg-slate-700 text-slate-400"}`}
                      >
                        <FileText size={16} />
                      </div>
