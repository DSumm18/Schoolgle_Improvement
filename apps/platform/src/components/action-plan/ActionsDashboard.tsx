...
export default function ActionsDashboard({
  ...
  const filteredActions = useMemo(() => {
    let result = actions.filter((action) => {
      if (filterUserStatus !== "all" && action.user_status !== filterUserStatus)
        return false;
      if (filterAIStatus !== "all" && action.ai_status !== filterAIStatus)
        return false;
      if (searchQuery) {
  ...
  const getStatusDisplay = (action: Action) => {
    ...
    return {
      ...
      color: colorMap[matrix.color] || colorMap.gray,
    };
  };
                        ...
                        >
                          <td className="px-4 py-4">
                            <div className="flex flex-col">
                              ...
                              {action.description && (
                                <span className="text-xs text-slate-500 line-clamp-1 mt-1 max-w-md">
                                  {action.description}
                                </span>
                              )}
                              {action.eef_strategy && (
                                ...
                                <div className="text-sm font-medium">
                                  ...
                                  {action.actual_cost && (
                                    <span className="text-xs text-slate-400 ml-1">
                                      (spent: £
                                      {action.actual_cost.toLocaleString()})
                                    </span>
                                  )}
                                </div>
                              ) : (
                          ...
                          <td className="px-4 py-4">
                            {eefStrategy ? (
                              <div className="flex items-center gap-2">
                                <span
                                  className={`text-[10px] font-bold uppercase ${EVIDENCE_COLORS[eefStrategy.evidenceStrength] || "text-slate-500"}`}
                                >
                          ...
                          <td className="px-4 py-4">
                            <span
                              className={`px-2 py-1 text-[10px] font-medium rounded-lg ${getPriorityStyles(action.priority)}`}
                            >
                              {action.priority}
                            </span>
                          </td>
                          <td className="px-4 py-4">
                            <span
                              className={`px-3 py-1 text-[10px] font-medium rounded-lg ${statusDisplay.color}`}
                            >
                              {statusDisplay.display}
                            </span>
                            <div className="text-[10px] text-slate-400 mt-0.5">
                              {action.user_status.replace("_", " ")} /{" "}
                              {action.ai_status.replace("_", " ")}
                            </div>
                          </td>
                          <td className="px-4 py-4">
                            {action.due_date ? (
                              <div
                                className={`flex items-center gap-1.5 text-sm font-medium ${new Date(action.due_date) < new Date() && action.user_status !== "complete" ? "text-rose-600" : "text-slate-700"}`}
                              >
          ...
          >
            <ActionsGanttChart
              ...
              onUpdateDates={(id, start, due) => {
                ...
                if (action) {
                  onUpdateAction({ ...action, due_date: new Date(due) });
                }
              }}
            />
          </motion.div>
