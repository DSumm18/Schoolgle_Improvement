...
export default function ActionModal({
  ...
  const [status, setStatus] = useState<
    "open" | "in_progress" | "completed" | "not_started" | "draft"
  >("open");
  ...
  const [linkedEvidence, setLinkedEvidence] = useState<
    { documentId: string; documentName: string; matchedAt: string }[]
  >([]);
  ...
  const validate = () => {
    ...
    if (!selectedCategory && !categoryName)
      newErrors.category = "Please assign a framework area";
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xl flex items-center justify-center z-[100] p-4">
      <motion.div
        ...
        className="bg-white dark:bg-slate-900 rounded-2xl shadow-[0_30px_100px_rgba(0,0,0,0.2)] w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col border border-white/20"
      >
        {/* Header */}
        <div className="px-10 py-8 border-b border-slate-100 dark:border-slate-800 flex justify-between items-start bg-slate-50/50 dark:bg-slate-800/50">
          <div>
            ...
            {errors.category && (
              <p className="text-[10px] text-rose-500 font-bold mt-2 uppercase tracking-widest">
                {errors.category}
              </p>
            )}
          </div>
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-slate-600 p-3 rounded-2xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-all"
          >
            <X size={24} />
          </button>
        </div>
          ...
          <form id="action-form" onSubmit={handleSubmit} className="space-y-10">
              ...
              <div className="space-y-4 bg-indigo-50/50 dark:bg-indigo-900/10 p-6 rounded-2xl border border-indigo-100 dark:border-indigo-800">
                ...
                <div className="space-y-2">
                  {linkedEvidence.map((ev, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between bg-white dark:bg-slate-800 p-3 rounded-xl border border-indigo-50 dark:border-indigo-900/50 shadow-sm"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] text-slate-400">
                            Detected on{" "}
                            {new Date(ev.matchedAt).toLocaleDateString()}
                          </p>
                        </div>
                      </div>
                      <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-slate-50/50 dark:bg-slate-800/30 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800">
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {["not_started", "in_progress", "completed", "open"].map(
                    (s) => (
                      <button
                        key={s}
                        type="button"
                        onClick={() => setStatus(s as any)}
                        className={`px-4 py-3 rounded-xl text-xs font-black uppercase tracking-tighter border-2 transition-all ${
                          status === s
            ...
            <div className="space-y-4 bg-slate-50/50 dark:bg-slate-800/30 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800">
              <div className="flex justify-between items-center mb-2">
                ...
                <span
                  className={`text-xs font-black ${progress === 100 ? "text-emerald-500" : "text-blue-600"}`}
                >
                  {progress}% Complete
                </span>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 bg-slate-50/50 dark:bg-slate-800/30 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800">
              <div className="space-y-4">
                ...
                <p className="text-[10px] text-slate-400 font-medium">
                  Hold Ctrl/Cmd to select multiple predecessor tasks
                </p>
              </div>
              ...
              <div className="space-y-4">
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`p-2 rounded-xl ${isCritical ? "bg-rose-600 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-400"}`}
                    >
                      <Sparkles size={16} />
                    </div>
                    <div className="text-left">
                      ...
                      <p className="text-[10px] font-bold opacity-60">
                        Prioritize in Gantt & Alerts
                      </p>
                    </div>
                  </div>
                  <div
                    className={`w-10 h-6 rounded-full p-1 transition-colors ${isCritical ? "bg-rose-600" : "bg-slate-200 dark:bg-slate-700"}`}
                  >
                    <div
                      className={`w-4 h-4 bg-white rounded-full transition-transform ${isCritical ? "translate-x-4" : ""}`}
                    />
                  </div>
                </button>
              </div>
                ...
                {notes.length > 0 && (
                  <div className="space-y-4 pl-4 border-l-2 border-slate-100 dark:border-slate-800">
                    {notes.map((note) => (
                      <div
                        key={note.id}
                        className="relative bg-slate-50 dark:bg-slate-800/20 p-5 rounded-2xl group/note"
                      >
                        <div className="flex justify-between items-center mb-2">
