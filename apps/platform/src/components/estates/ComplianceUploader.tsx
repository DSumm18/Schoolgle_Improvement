...
export default function ComplianceUploader({
  ...
  const processUpload = async () => {
    ...
    try {
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from("compliance").getPublicUrl(filePath);

      // 4. Call Edge Function ("The Clerk" & "The Auditor")
      setStatus("ANALYZING");

      const { data, error: functionError } = await supabase.functions.invoke(
        "process-compliance-doc",
        {
          body: {
            file_url: publicUrl,
            user_id: user.id,
            ...
            <>
              ...
              <div className="space-y-2">
                <h3 className="font-semibold text-xl">
                  Paste-to-See Verification
                </h3>
                <p className="text-zinc-500 dark:text-zinc-400 max-w-xs text-sm">
                  Drag and drop your {domainName} certificate here, or press{" "}
                  <kbd className="px-1.5 py-0.5 rounded border bg-zinc-100 dark:bg-zinc-800 text-[10px]">
                    Ctrl+V
                  </kbd>{" "}
          ...
          {file && status === "IDLE" && (
            <div className="w-full space-y-4">
              ...
              <Button
                ...
                className="w-full bg-teal-600 hover:bg-teal-700 text-white shadow-md shadow-teal-500/20 h-12 text-lg"
              >
            ...
            <div className="flex flex-col items-center space-y-4 py-6">
              ...
              <div className="space-y-1">
                <p className="font-medium text-lg">
                  {status === "UPLOADING"
                    ? "Ingesting Document..."
                    : 'Agent "Ed" is verifying...'}
                </p>
                <p className="text-sm text-zinc-500">
                  {status === "UPLOADING"
                    ? "Storing in secure vaults"
                    : "Reading taxonomy & checking rules"}
                </p>
              </div>
            </div>
          )}
          ...
          {status === "SUCCESS" && (
            <div className="flex flex-col items-center space-y-4 py-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm text-zinc-500">
                  Digital proof has been filed against this asset.
                </p>
              </div>
              <Button
                variant="outline"
