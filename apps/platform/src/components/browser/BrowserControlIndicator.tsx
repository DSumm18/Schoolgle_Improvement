        ...
        >
          ...
          <div className="p-4 border-b border-current border-opacity-10">
            <div className="flex items-center gap-3">
              {/* Animated Icon */}
              <div
                className={`relative ${isPulsing && isActive ? "animate-pulse" : ""}`}
              >
                <div
                ...
                ></div>
                <Icon
                  className={`w-6 h-6 relative ${isActive ? "animate-spin" : ""}`}
                />
              </div>
...
export function MinimalBrowserControl({
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999]">
      <div
        className="bg-sky-600 text-white rounded-full shadow-lg px-4 py-2 flex items-center gap-2 animate-pulse"
        style={{
          backdropFilter: "blur(12px)",
          WebkitBackdropFilter: "blur(12px)",
        }}
    ...
    <div className="fixed inset-0 z-[9997] flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-md mx-4 text-center">
        ...
        {canDismiss && (
          <button
            onClick={onDismiss}
            className="w-full py-2 px-4 rounded-lg bg-sky-600 hover:bg-sky-700 text-white font-medium transition-colors"
          >
            Got it, thanks
          </button>
        )}
...
export function useUserInterferenceDetection(
  enabled: boolean,
  onInterfere: () => void,
) {
  useEffect(() => {
