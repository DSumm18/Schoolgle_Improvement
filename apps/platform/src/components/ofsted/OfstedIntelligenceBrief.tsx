...
interface CohortGapEvidenceSummary {
  source: string;
  caveat: string;
...
export default function OfstedIntelligenceBrief({
  ...
  const [cohortGapEvidence, setCohortGapEvidence] =
    useState<CohortGapEvidenceSummary | null>(null);
  ...
  const loadBrief = useCallback(async () => {
    ...
    try {
      ...
      setCohortGapEvidence(
        readinessResponse.ok ? readinessPayload.cohortGapEvidence ?? null : null,
      );
    } catch (err) {
  ...
  return (
    <Card className="border-border/70 bg-card shadow-sm">
      ...
      <CardContent className="space-y-5">
        ...
        {cohortGapEvidence && cohortGapEvidence.comparisons.length > 0 && (
          <div className="rounded-xl border border-indigo-200 bg-indigo-50/70 p-4 text-indigo-950 dark:border-indigo-900/70 dark:bg-indigo-950/25 dark:text-indigo-100">
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.14em] opacity-80">
                  Trust Assessor Cohort Gap Evidence
                </p>
                <h3 className="mt-1 text-lg font-bold">
                  {cohortGapEvidence.comparisons.length} subgroup pattern
                  {cohortGapEvidence.comparisons.length === 1 ? "" : "s"} ready
                  for Ofsted evidence review
                </h3>
                <p className="mt-2 text-sm leading-relaxed">
                  These are the clean “why might the headline look like this?”
                  signals from Trust Assessor. Use them to attach books,
                  provision records, intervention evidence and governor
                  challenge notes.
                </p>
              </div>
              <div className="rounded-lg bg-white/70 px-3 py-2 text-sm font-semibold text-indigo-800 dark:bg-slate-950/40 dark:text-indigo-100">
                {cohortGapEvidence.latestYear
                  ? `${cohortGapEvidence.latestYear}/${String(
                      cohortGapEvidence.latestYear + 1,
                    ).slice(-2)}`
                  : "Latest import"}
              </div>
            </div>
            ...
            <div className="mt-4 grid gap-3 lg:grid-cols-3">
              {cohortGapEvidence.comparisons.slice(0, 3).map((comparison) => (
                <div
                  key={`${comparison.organizationId}-${comparison.key}`}
                  className="rounded-lg border border-indigo-200/80 bg-white/75 p-3 text-sm dark:border-indigo-900/70 dark:bg-slate-950/30"
                >
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        {comparison.yearGroupLabel}
                        {comparison.assessmentPeriod
                          ? ` · ${comparison.assessmentPeriod}`
                          : ""}
                        {comparison.academicYearStart
                          ? ` · ${comparison.academicYearStart}/${String(
                              comparison.academicYearStart + 1,
                            ).slice(-2)}`
                          : ""}
                      </p>
                    </div>
                    <span className="rounded-full border border-indigo-200 bg-indigo-100 px-2 py-0.5 text-[11px] font-bold text-indigo-800 dark:border-indigo-800 dark:bg-indigo-900/50 dark:text-indigo-100">
                      {comparison.combinedGapPp !== null
                        ? `${comparison.combinedGapPp > 0 ? "+" : ""}${comparison.combinedGapPp}pp`
                        : "—"}
                    </span>
                  </div>
                  ...
                  <p className="mt-2 text-[11px] font-semibold uppercase tracking-wide text-indigo-700 dark:text-indigo-200">
                    Ofsted area: {comparison.ofstedArea}
                  </p>
                </div>
              ))}
            </div>

            <div className="mt-3 space-y-1 border-t border-indigo-200/80 pt-3 text-xs dark:border-indigo-900/70">
              <p>
                <span className="font-semibold">Source:</span>{" "}
                {cohortGapEvidence.source}
              </p>
              <p>
                <span className="font-semibold">Caveat:</span>{" "}
                {cohortGapEvidence.caveat}
              </p>
            </div>
          </div>
        )}
