        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-6 pt-8">
              <Link
                href="/early-access"
                className="px-10 py-5 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-full font-black text-xs uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-xl hover:shadow-2xl"
              >
                Book a demo
              </Link>
              <Link
                href="/modules"
                className="px-10 py-5 bg-background text-slate-900 dark:text-white rounded-full font-black text-xs uppercase tracking-widest border border-slate-100 dark:border-white/10 hover:border-slate-300 dark:hover:border-white/20 transition-all"
              >
                Back to Ecosystem
              </Link>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-start">
            <div className="space-y-6 lg:sticky lg:top-40">
              ...
              <p className="text-xl text-slate-500 dark:text-slate-400 font-medium leading-relaxed max-w-md">
                Ed acts as your always-available specialist, guiding your team
                through {theme.name.toLowerCase()} tasks without the need for
                constant training.
              </p>
            </div>
                ...
                >
                  <div className="flex flex-col sm:flex-row gap-8 items-start">
                    <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-white/5 flex items-center justify-center shrink-0 group-hover:bg-[var(--module-accent-soft)] transition-colors">
                      <span className="text-xs font-black text-slate-400 dark:text-slate-500 group-hover:text-[var(--module-accent)]">
                        0{i + 1}
                      </span>
                    </div>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-24 space-y-4">
            ...
            <h2 className="text-4xl md:text-5xl font-black text-slate-900 dark:text-white uppercase tracking-tight">
              Typical jobs Ed completes with you
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {typicalJobs.map((job, i) => (
              <div
                key={i}
                className="p-10 rounded-3xl bg-slate-50/50 dark:bg-white/5 border border-slate-100/50 dark:border-white/10 hover:border-[var(--module-accent)] dark:hover:border-[var(--module-accent)] transition-all group"
              >
                <h4 className="text-base font-black text-slate-900 dark:text-white uppercase tracking-tight mb-4 group-hover:text-[var(--module-accent)] transition-colors">
                  {job}
                </h4>
          ...
          <div className="space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-6">
              {whatItCovers.map((item, i) => (
                <div
                  key={i}
                  className="flex gap-4 items-center py-4 border-b border-slate-100 dark:border-white/5"
                >
      ...
      <section className="py-24 px-6 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-[4rem] mx-6 mb-24 overflow-hidden relative transition-colors duration-700">
        <div className="max-w-4xl mx-auto relative z-10 text-center space-y-8">
          ...
          <p className="text-slate-400 dark:text-slate-600 font-medium text-lg max-w-2xl mx-auto">
            Like every Schoolgle module, {theme.name} uses Guardian Mode to
            prevent risks. No data is moved from your existing systems of
            record.
          </p>
        </div>
