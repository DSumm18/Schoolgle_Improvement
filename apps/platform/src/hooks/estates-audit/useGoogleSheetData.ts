...
const parseSheetData = (
  ...
  const kpiCategories: KpiCategory[] = Object.entries(categories).map(
  ...
  const overallScore =
    kpiCategories.length > 0
      ? Math.round(
          kpiCategories.reduce((sum, category) => sum + category.average, 0) /
            kpiCategories.length,
        )
      : 0;
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          `https://sheets.googleapis.com/v4/spreadsheets/${(config as any).SHEET_ID}?key=${(config as any).API_KEY}`,
        );
        ...
        for (const sheet of sheets) {
          ...
          if (
            IGNORED_SHEET_NAMES.some((ignored) =>
          ...
          try {
            const sheetResponse = await fetch(
              `https://sheets.googleapis.com/v4/spreadsheets/${(config as any).SHEET_ID}/values/${encodeURIComponent(sheetTitle)}?key=${(config as any).API_KEY}`,
            );

            if (!sheetResponse.ok) {
              console.warn(
                `Failed to fetch sheet ${sheetTitle}: ${sheetResponse.statusText}`,
              );
              continue;
            }
