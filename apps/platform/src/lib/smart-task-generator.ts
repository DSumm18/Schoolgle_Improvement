...
export function generateSmartTasks(
    ...
    category.subcategories.forEach((sub) => {
      ...
      if (coverage < 70) {
        ...
        sub.evidenceRequired.forEach((requiredItem) => {
          ...
          if (!hasMatch) {
            const isPolicy =
              requiredItem.name.toLowerCase().includes("policy") ||
            ...
            tasks.push({
              ...
              title: isPolicy
                ? `Critical: Policy Missing - ${requiredItem.name}`
                : `Upload Evidence: ${requiredItem.name}`,
              ...
              status: "draft",
              due_date: new Date(
                Date.now() + (isPolicy ? 7 : 14) * 24 * 60 * 60 * 1000,
              ).toISOString(),
              source: "scan_gap",
  ...
  allMatches.forEach((match) => {
    if (match.documentModifiedTime && !processedDocs.has(match.documentId)) {
      ...
      if (modDate < twelveMonthsAgo) {
        const docAgeMonths = Math.floor(
          (now.getTime() - modDate.getTime()) / (1000 * 60 * 60 * 24 * 30),
        );
          ...
          linked_evidence: [
            {
              ...
              matchedAt: (match as any).matchedAt,
            },
          ],
          created_at: new Date().toISOString(),
        });
  ...
  Object.values(assessmentUpdates).forEach((update) => {
    if (
      update.aiRating === "requires_improvement" ||
      update.aiRating === "inadequate"
    ) {
