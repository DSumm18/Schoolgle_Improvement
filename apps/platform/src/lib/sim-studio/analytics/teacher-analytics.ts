...
export function generatePupilCard(
  ...
  const skillsWithData = skillSnapshots.filter(
    (s) => s.confidence !== "insufficient_data",
  );
  ...
  const weakAreas = skillsWithData.filter(
    (s) => (s.concept_score || 0) < 60 || (s as any).transfer_gap === true,
  );
  ...
  const scaffoldRecommendation = recommendOverallScaffold(
    pupilProfile,
    questRuns,
  );
  ...
  const skillsWithEvidence = skillSnapshots.filter(
    (s) => s.evidence_coverage >= 70,
  ).length;
  const evidenceCoverage =
    totalSkills > 0 ? (skillsWithEvidence / totalSkills) * 100 : 0;

  // Calculate average score
  const avgScore =
    questRuns.length > 0
      ? questRuns.reduce((sum, run) => sum + run.total_score, 0) /
        questRuns.length
      : 0;
  ...
  const lastQuestDate =
    questRuns.length > 0
      ? questRuns[questRuns.length - 1].completed_at
      : undefined;
...
export function generateClassHeatmap(
  ...
  const pupils = pupilProfiles.map((profile) => {
    ...
    const totalScore =
      pupilSkills.length > 0
        ? pupilSkills.reduce((sum, s) => sum + (s.concept_score || 0), 0) /
          pupilSkills.length
        : 0;

    const skillsRecord: Record<string, SkillSnapshot> = {};
    pupilSkills.forEach((s) => {
      skillsRecord[s.skill] = s;
    });
  ...
  // Get unique skills
  const skills = Array.from(new Set(skillSnapshots.map((s) => s.skill))).sort();
...
export function generateEvidenceWarnings(
  ...
  const pupilsWithLowCoverage = pupilProfiles.filter((profile) => {
    const pupilSnapshots = skillSnapshots.filter(
      (s) => s.pupil_id === profile.id,
    );
    const avgCoverage =
      pupilSnapshots.length > 0
        ? pupilSnapshots.reduce((sum, s) => sum + s.evidence_coverage, 0) /
          pupilSnapshots.length
        : 0;
  ...
  if (pupilsWithLowCoverage.length > 0) {
    warnings.push({
      ...
      severity: "high",
      affectedEntities: pupilsWithLowCoverage.map(
  ...
  >();
  skillSnapshots.forEach((snapshot) => {
    const current = skillCoverage.get(snapshot.skill) || {
      total: 0,
      withEvidence: 0,
    };
    current.total++;
  ...
  if (sendPupils.length > 0) {
    ...
    const avgSendCoverage =
      sendSnapshots.length > 0
        ? sendSnapshots.reduce((sum, s) => sum + s.evidence_coverage, 0) /
          sendSnapshots.length
        : 0;
    ...
    if (avgSendCoverage < 50) {
      warnings.push({
        ...
        suggestedAction:
          "Ensure SEND pupils are engaged with appropriate quests",
      });
    }
  }
  ...
  if (ealPupils.length > 0) {
    ...
    const avgEalCoverage =
      ealSnapshots.length > 0
        ? ealSnapshots.reduce((sum, s) => sum + s.evidence_coverage, 0) /
          ealSnapshots.length
        : 0;
...
export function generateCalibrationData(
  ...
  teacherJudgements.forEach((judgement) => {
    ...
    const snapshot = skillSnapshots.find(
      (s) => s.pupil_id === judgement.pupil_id && s.skill === skillKey,
    );
    ...
    if (snapshot && snapshot.confidence !== "insufficient_data") {
      ...
      const status =
        judgement.judgement === schoolgleJudgement
          ? "agreement"
...
function recommendOverallScaffold(
  profile: PupilProfile,
  questRuns: QuestRun[],
): string {
