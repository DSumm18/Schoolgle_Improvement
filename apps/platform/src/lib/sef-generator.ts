...
export const SEFGenerator = {
  ...
  async generateSection(
    ...
    const relevantAssessments = data.assessments.filter((a) =>
      a.subcategory_id?.startsWith(sectionId),
    );
    const relevantEvidence = data.evidence.filter((e) =>
      e.subcategory_id?.startsWith(sectionId),
    );
    const relevantActions = data.actions.filter(
      (a) =>
        a.category_id === sectionId || a.subcategory_id?.startsWith(sectionId),
    );
    ...
    return {
      ...
      strengths: relevantAssessments
        .filter(
          (a) =>
            a.school_rating === "exceptional" ||
            a.school_rating === "strong_standard",
        )
      ...
      afd: relevantAssessments
        .filter(
          (a) =>
            a.school_rating === "needs_attention" ||
            a.school_rating === "urgent_improvement",
        )
        .map((a) => a.school_rationale)
        .filter(Boolean),
      ...
      impact:
        "Analysis of internal tracking and previous inspection outcomes confirms sustained improvement in this area.",
      nextSteps:
        "Continue periodic reviews to ensure consistency across all departments.",
    };
  },

  calculateGrade(assessments: any[]): string {
    if (assessments.length === 0) return "not_assessed";

    const scores = assessments
      .map((a) => {
        const rating =
          OFSTED_RATINGS[a.school_rating as keyof typeof OFSTED_RATINGS];
  ...
  buildNarrative(
    ...
    const phrases =
      OFSTED_PHRASES[grade as keyof typeof OFSTED_PHRASES] ||
      OFSTED_PHRASES.strong_standard;
    const randomPhrase = () =>
  ...
  exportToMarkdown(sef: any): string {
    return (
      `# Self-Evaluation Form (${sef.academicYear})\n\n` +
      sef.sections
        .map(
          (s: any) => `
