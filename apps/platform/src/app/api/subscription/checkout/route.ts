  ...
  try {
    ...
    if (!stripeSecretKey || !stripeEnabled) {
      return NextResponse.json(
        {
          error:
            "Card payments are not currently available. Please use the invoice payment option.",
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url:
        successUrl ||
        `${process.env.NEXT_PUBLIC_APP_URL}/dashboard/account?upgraded=true`,
      cancel_url:
        cancelUrl ||
        `${process.env.NEXT_PUBLIC_APP_URL}/dashboard/account/upgrade`,
      metadata: {
        organizationId,
        planId,
      },
