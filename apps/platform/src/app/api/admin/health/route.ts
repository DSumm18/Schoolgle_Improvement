...
// Calculate health score for all organizations (admin only)
  ...
  async (auth, req) => {
    try {
      ...
      for (const org of orgs || []) {
        ...
        const totalAiCost =
          usageData?.reduce(
            (sum, d) => sum + (parseFloat(d.ai_total_cost_usd) || 0),
            0,
          ) || 0;
        const totalAiQueries =
          usageData?.reduce((sum, d) => sum + (d.ai_chats || 0), 0) || 0;
        const uniqueActiveUsers = new Set(activeUsers?.map((u) => u.user_id))
          .size;
        const activeUsersPercent = totalUsers
          ? (uniqueActiveUsers / totalUsers) * 100
          : 0;
        ...
        const daysSinceLogin = lastLogin
          ? Math.floor(
              (Date.now() - new Date(lastLogin.created_at).getTime()) /
                (1000 * 60 * 60 * 24),
            )
          : 999;
        ...
        let adoptionScore = Math.min(
          100,
          featuresUsed * 15 + activeUsersPercent * 0.5,
        );

        let valueScore = Math.min(
          100,
          completionRate * 0.5 +
            (totalAiQueries > 0 ? 30 : 0) +
            Math.min(20, daysActive),
        );

        const healthScore = Math.round(
          engagementScore * 0.4 + adoptionScore * 0.3 + valueScore * 0.3,
        );
...
// Get health data for admin dashboard (admin only)
