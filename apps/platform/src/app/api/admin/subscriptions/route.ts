  ...
  async (auth, req) => {
    ...
    if (health && health !== "all") {
      filteredData = data?.filter(
        (sub) => sub.health?.[0]?.health_status === health,
      );
    }
    ...
    const summary = {
      ...
      cancelled:
        filteredData?.filter((s) => s.status === "cancelled").length || 0,
      mrr: Math.round(
        filteredData
    ...
    return apiSuccess({ data: filteredData, summary });
  },
  { requiredRole: "admin" },
);
  ...
  async (auth, req) => {
    ...
    switch (action) {
      ...
      case "downgrade":
        ...
        const newPrice =
          prices[newPlan as keyof typeof prices] * currentSub.school_count;
