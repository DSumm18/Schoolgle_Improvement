...
export async function GET(req: NextRequest) {
    try {
        ...
        const enrichedAssessments = (assessments || []).map((assessment) => {
            const categoryInfo = OFSTED_JUDGEMENTS[assessment.category_id as OfstedCategoryId];
            const subcategoryInfo = OFSTED_SUBCATEGORIES[assessment.subcategory_id as OfstedSubCategoryId];
        ...
        const categoriesMap = new Map<OfstedCategoryId, {
        ...
        for (const category of categoriesMap.values()) {
            ...
            if (categoryAssessments.length > 0) {
                const totalScore = categoryAssessments.reduce((sum, a) => sum + calculateReadinessScore(a), 0);
                category.average_score = Math.round(totalScore / categoryAssessments.length);

                // Calculate average rating
                const ratedAssessments = categoryAssessments.filter(a => a.school_rating);
                if (ratedAssessments.length > 0) {
                    ...
                    else if (avgRating >= 2.5) category.average_rating = 'expected_standard';
        ...
        const totalEvidence = categorySummaries.reduce((sum, cat) => sum + cat.total_evidence, 0);
        ...
        console.error('Ofsted assessments API error:', error);
        return NextResponse.json(
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!organization_id || !assessments || assessments.length === 0) {
            return NextResponse.json(
                { error: 'Missing required fields: organization_id, assessments' },
                { status: 400 }
            );
        }
        ...
        console.error('Ofsted upsert API error:', error);
        return NextResponse.json(
            { error: 'Internal server error' },
            { status: 500 }
        );
