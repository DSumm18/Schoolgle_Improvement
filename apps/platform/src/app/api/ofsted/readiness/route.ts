...
export async function GET(req: NextRequest) {
    try {
        ...
        if (assessmentsError) {
            console.error('Error fetching Ofsted assessments for readiness:', assessmentsError);
            return NextResponse.json(
        ...
        for (const assessment of assessments || []) {
            const category = categoriesMap.get(assessment.category_id as OfstedCategoryId);
            if (category) {
            ...
            if (includeGaps) {
                const subcategoryInfo = OFSTED_SUBCATEGORIES[assessment.subcategory_id as keyof typeof OFSTED_SUBCATEGORIES];
                const isCritical = !assessment.school_rating || assessment.school_rating === 'needs_attention' || assessment.school_rating === 'urgent_improvement';
                const hasNoEvidence = assessment.evidence_count === 0;

                if (isCritical || hasNoEvidence) {
                    const gapLevel: 'critical' | 'moderate' | 'none' = isCritical && hasNoEvidence ? 'critical' : isCritical ? 'moderate' : 'none';

                    gaps.push({
                        category_id: assessment.category_id as OfstedCategoryId,
                        subcategory_id: assessment.subcategory_id as keyof typeof OFSTED_SUBCATEGORIES,
                        ...
                        missing_evidence: hasNoEvidence ? ['No evidence found for this subcategory'] : [],
                    });
                }
            }
        }
        ...
        for (const category of categoriesMap.values()) {
            ...
            if (scores.length > 0) {
                category.average_score = Math.round(scores.reduce((a, b) => a + b, 0) / scores.length);
        ...
        if (snapshots && snapshots.length > 1) {
            ...
            for (const cat of Object.keys(categoryChanges) as OfstedCategoryId[]) {
                categoryChanges[cat] = (latest.category_scores[cat] || 0) - (previous.category_scores[cat] || 0);
            }
        ...
        console.error('Ofsted readiness API error:', error);
        return NextResponse.json(
...
export async function POST(req: NextRequest) {
    try {
        ...
        const { data: readinessData } = await supabase
            .rpc('calculate_ofsted_readiness', { p_org_id: organization_id });
        ...
        return NextResponse.json(createdSnapshot);
    } catch (error) {
        console.error('Ofsted snapshot API error:', error);
        return NextResponse.json(
            { error: 'Internal server error' },
            { status: 500 }
        );
