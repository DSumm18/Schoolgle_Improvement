...
export async function POST(req: NextRequest) {
  try {
    ...
    if (urn) {
      result = (await lookupSchoolByURN(urn)) || result;
    } else if (school_name) {
      const la = la_code || (await findLABySchoolName(school_name));
      result = (await lookupSchoolByNameAndLA(school_name, la || "")) || result;
    }
 ...
 */
async function lookupSchoolByURN(
  urn: string,
): Promise<DfeSchoolLookupResponse | null> {
  try {
    const response = await fetch(
      `${GIAS_API_BASE}/establishments/${urn}.json`,
      {
        headers: {
