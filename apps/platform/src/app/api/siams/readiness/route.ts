...
export async function GET(req: NextRequest) {
    try {
        ...
        if (fromDate) {
            assessmentsQuery = assessmentsQuery.gte('updated_at', fromDate);
        }

        const { data: assessments, error: assessmentsError } = await assessmentsQuery;

        if (assessmentsError) {
            console.error('Error fetching SIAMS assessments:', assessmentsError);
            return NextResponse.json(
        ...
        const evidenceByQuestion = (evidenceMatches || []).reduce((acc: any, ev: any) => {
            acc[ev.question_id] = (acc[ev.question_id] || 0) + 1;
            return acc;
        }, {} as Record<SiamsQuestionId, number>);

        // Build assessments map
        const assessmentsMap = new Map<SiamsQuestionId, {
        ...
        (assessments || []).forEach((a: any) => {
            assessmentsMap.set(a.question_id as SiamsQuestionId, {
                ...
                evidence_count: a.evidence_count || evidenceByQuestion[a.question_id] || 0,
            });
        });
        ...
        const totalEvidence = (assessments || []).reduce((sum: number, a: any) =>
            sum + (a.evidence_count || evidenceByQuestion[a.question_id] || 0), 0
        );
        ...
        Object.values(SIAMS_STRANDS).forEach((strand) => {
            ...
            if (assessedCount > 0) {
                strandScores[strand.id as SiamsStrandId] = Math.round(strandTotal / assessedCount);
        ...
        const overallScore = strandCount > 0 ? Math.round(totalScore / strandCount) : 0;
        const overallRating = calculateRatingFromScore(overallScore);

        // Build strand summaries
        const strands: SiamsStrandSummary[] = Object.values(SIAMS_STRANDS).map((strand) => {
            const strandQuestions = Object.values(SIAMS_QUESTIONS)
                .filter((q) => q.strand === strand.id)
                .map((q) => q.id);
        ...
        Object.values(SIAMS_QUESTIONS).forEach((question) => {
            ...
            if (schoolRating === 'requires_improvement' || schoolRating === 'ineffective') {
                needsAttention = true;
            ...
            if (gapLevel !== 'none' || needsAttention) {
                gaps.push({
                    ...
                    missing_evidence: evidenceCount === 0
                        ? ['No evidence linked to this question']
                        : [],
                });
            }
        });
            ...
            if (snapshots && snapshots.length >= 2) {
                ...
                Object.keys(latest.strand_scores || {}).forEach((strandId) => {
                    ...
                    if (latestScore !== undefined && previousScore !== undefined) {
                        strandChanges[strandId as SiamsStrandId] = latestScore - previousScore;
                    }
                });
        ...
        console.error('SIAMS Readiness API error:', error);
        return NextResponse.json(
...
export async function POST(req: NextRequest) {
    try {
        ...
        const readinessResponse = await GET(req);
        if (!readinessResponse.ok) {
            return readinessResponse;
        }

        const readinessData = await readinessResponse.json() as GetSiamsReadinessResponse;
        ...
        console.error('SIAMS snapshot creation error:', error);
        return NextResponse.json(
            { error: error.message || 'Internal server error' },
            { status: 500 }
        );
...
function getAverageRating(
    ...
    const ratings = assessments
        ...
        .filter((r): r is SiamsRating => r !== null && r !== undefined) as SiamsRating[];
    ...
    return Object.entries(counts).sort((a, b) => b[1] - a[1])[0][0] as SiamsRating;
}
