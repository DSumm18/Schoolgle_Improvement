...
import type { TaskTemplate, TaskTemplateForm, ActionChecklistItem } from '@/lib/tasks';
        ...
        const results = await Promise.all(
            updates.map(async ({ id, changes }) => {
                ...
                if (changes.checklist_template) {
                    updateData.checklist_template = changes.checklist_template.map((item) => ({
                        ...item,
                        id: item.id || uuidv4(),
                    }));
                }
...
export async function applyTemplate(req: NextRequest) {
    try {
        ...
        const dueDate = custom_due_date
            ? custom_due_date
            : new Date(Date.now() + (template.default_due_days || 14) * 24 * 60 * 60 * 1000)
                .toISOString()
                .split('T')[0];

        // Create task from template
        const taskId = uuidv4();
        const checklistWithIds = (template.checklist_template || []).map((item) => ({
            ...item,
            id: uuidv4(),
            completed: false,
            completed_by: null,
        ...
        const { data: task, error: taskError } = await supabase
            ...
            .insert({
                ...
                approval_status: template.requires_approval ? 'pending_approval' : 'approved',
                progress: 0,
                status: 'not_started',
                linked_evidence: [],
                notes: [],
