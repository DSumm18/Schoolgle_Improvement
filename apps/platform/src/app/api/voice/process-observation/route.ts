...
export const POST = aiRoute(async (auth, req) => {
  const body = await req.json();
  const { transcript, teacherName, subject, yearGroup } = body;
...
Return ONLY valid JSON, no markdown formatting.`;
  ...
  return apiSuccess({
    ...
    transcript,
    summary:
      "A well-structured lesson demonstrating effective teaching practices with clear opportunities for development.",
    strengths: [
