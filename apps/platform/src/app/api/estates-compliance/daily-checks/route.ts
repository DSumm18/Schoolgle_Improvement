  ...
  try {
    ...
    const routinesForToday = (activeRoutines || []).filter((routine) => {
      ...
      if (routine.recurrence === "weekly") {
        return (
          routine.recurrence_days && routine.recurrence_days.includes(dayOfWeek)
        );
      }
    ...
    const { data: completions, error } = await query.order("created_at", {
      ascending: false,
    });
    ...
    const openingCompletion = completions?.find(
      (c) => c.check_type === "opening",
    );
    const closingCompletion = completions?.find(
      (c) => c.check_type === "closing",
    );
    ...
    routinesForToday.forEach((routine) => {
      ...
      dynamicStatuses[routine.id] = {
        ...
        completed:
          ...
          completion?.status === "completed_with_issues",
    ...
    const openingStatus = getChecklistStatusForToday(
      completions || [],
      "opening",
    );
    const closingStatus = getChecklistStatusForToday(
      completions || [],
      "closing",
    );
...
export const POST = protectedRoute(
  async (auth, request) => {
    try {
      ...
      const { check_type, action, results, notes, photos, routine_id } = body;

      const organization_id = auth.organizationId;
      const user_id = auth.userId;

      if (!check_type && !routine_id) {
        return apiError("check_type or routine_id is required", 400);
      }
      ...
      if (action === "start" || action === "resume") {
        ...
        if (routine_id) {
          ...
          const { DAILY_CHECKLISTS } =
            await import("@/lib/estates-compliance/daily-checks");
          const checklist =
            DAILY_CHECKLISTS[check_type as "opening" | "closing"];
          if (checklist) {
