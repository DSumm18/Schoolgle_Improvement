    ...
    try {
        ...
        if (!organizationId || !csvData) {
            return NextResponse.json(
                { error: 'organizationId and csvData are required' },
                { status: 400 }
            );
        }
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                ...
                const normalizedData = {
                    ...
                    is_active: normalizeBoolean(row.is_active !== undefined ? row.is_active : true),
                ...
                const removeActions = ['remove', 'delete', 'archive', 'inactive', 'removed', 'archived', 'deleted'];
                ...
                if (removeActions.includes(action)) {
                    if (existingStaff) {
                        ...
                        results.warnings.push(`Row ${rowNum}: Staff member "${normalizedData.first_name} ${normalizedData.last_name}" not found - cannot remove`);
                    }
                    continue;
                }
        ...
        if (noEmailCount > 0) {
            results.warnings.push(`${noEmailCount} staff member(s) without email address`);
        }

        const noEmployeeIdCount = rows.filter(r => !r.employee_id).length;
        if (noEmployeeIdCount > 0) {
            results.warnings.push(`${noEmployeeIdCount} staff member(s) without employee ID`);
        }

        return NextResponse.json(results);
    } catch (error) {
        console.error('Error in POST /api/staff/import:', error);
...
export async function GET(request: NextRequest) {
    ...
    if (type === 'export' && organizationId) {
        ...
        if (!staff || staff.length === 0) {
            ...
            const headers = ['salutation', 'first_name*', 'last_name*', 'email', 'phone', 'employee_id', 'job_title*', 'role_category', 'is_super_user', 'is_active', 'action'];
            const csv = [
                instructions,
                headers.join(','),
            ].join('\n');
    ...
    if (type === 'template') {
        ...
        const headers = ['salutation', 'first_name*', 'last_name*', 'email', 'phone', 'employee_id', 'job_title*', 'role_category', 'is_super_user', 'is_active', 'action'];
        const exampleRows = [
            ['Mr', 'John', 'Smith', 'john.smith@school.co.uk', '01234 567890', 'STF001', 'Headteacher', 'headteacher', 'no', 'yes', 'new'],
            ['Mrs', 'Sarah', 'Jones', 'sarah.jones@school.co.uk', '', 'STF002', 'Deputy Headteacher', 'deputy_headteacher', 'no', 'yes', 'new'],
            ['Ms', 'Emily', 'Brown', 'emily.brown@school.co.uk', '', 'STF003', 'SENCO', 'sendco', 'no', 'yes', 'new'],
            ['', 'David', 'Wilson', 'david.wilson@school.co.uk', '', 'STF004', 'Class Teacher', 'class_teacher', 'no', 'yes', 'new'],
            ['', 'Jane', 'Doe', 'jane.doe@school.co.uk', '', 'STF005', 'Former Staff', 'support_staff', 'no', 'no', 'remove'],
        ];
        ...
        return new NextResponse(csv, {
            headers: {
                'Content-Type': 'text/csv',
                'Content-Disposition': 'attachment; filename="staff_directory_template.csv"',
            },
        });
    }
