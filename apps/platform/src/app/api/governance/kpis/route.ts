    ...
    try {
        ...
        const [
            ...
            supabase
                ...
                .select('*')
                .eq('organization_id', organizationId)
                .gte('scheduled_date', new Date(Date.now() - 365 * 24 * 60 * 60 * 1000).toISOString()),
        ...
        const governorTypes = governors.reduce((acc: any, g: any) => {
        ...
        const vacantPositions = activeGovernors.filter((g: any) =>
            g.end_date && new Date(g.end_date) < today
        ).length;

        // Calculate meeting statistics
        const upcomingMeetings = meetings.filter((m: any) =>
            new Date(m.scheduled_date) >= today && m.status === 'scheduled'
        ).length;
        const pastMeetingsThisYear = meetings.filter((m: any) =>
            new Date(m.scheduled_date) < today && m.status === 'completed'
        ).length;

        // Calculate average attendance
        let totalAttendanceRate = 0;
        const governorsWithAttendance = governors.filter((g: any) => g.meetings_total > 0);
        if (governorsWithAttendance.length > 0) {
            const attendanceSum = governorsWithAttendance.reduce((sum: number, g: any) => {
                return sum + (g.meetings_attended / g.meetings_total);
            }, 0);
            totalAttendanceRate = Math.round((attendanceSum / governorsWithAttendance.length) * 100);
        }

        // Calculate training statistics
        const todayStr = today.toISOString().split('T')[0];
        const expiredTraining = training.filter((t: any) =>
            t.expiry_date && t.expiry_date < todayStr
        ).length;
        const trainingCompletionRate = governors.length > 0
        ...
        const policiesCurrent = policies.filter((p: any) =>
            p.review_status === 'current' || p.next_review_date >= todayStr
        ).length;
        const policiesNeedReview = policies.filter((p: any) =>
            p.review_status === 'under_review' ||
            (p.review_status === 'current' && p.next_review_date < todayStr)
        ).length;
        const policiesOverdue = policies.filter((p: any) => p.next_review_date < todayStr).length;

        // Calculate visit statistics
        const visitsThisTerm = visits.length;
        const visitsCompleted = visits.filter((v: any) => v.status === 'completed').length;
        const visitsScheduled = visits.filter((v: any) => v.status === 'scheduled').length;
        ...
        if (snapshots.length >= 2) {
            ...
            } else if (latest.training_completion_rate < previous.training_completion_rate) {
                trends.training_trend = 'declining';
            }

            const latestPolicyRate = latest.policies_current / (latest.policies_current + latest.policies_outstanding_review);
            const previousPolicyRate = previous.policies_current / (previous.policies_current + previous.policies_outstanding_review);
        ...
        console.error('Governance KPIs API error:', error);
        return NextResponse.json(
...
export async function POST(req: NextRequest) {
    try {
        ...
        console.error('KPI snapshot creation error:', error);
        return NextResponse.json(
            { error: error.message || 'Internal server error' },
            { status: 500 }
        );
