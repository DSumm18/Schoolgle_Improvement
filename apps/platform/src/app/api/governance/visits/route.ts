...
export async function GET(req: NextRequest) {
    try {
        ...
        const scheduled = enrichedVisits.filter((v: any) => v.status === 'scheduled').length;
        const completed = enrichedVisits.filter((v: any) => v.status === 'completed').length;
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!organizationId || !governorId || !visit_type || !title || !scheduled_date) {
            return NextResponse.json(
                { error: 'Missing required fields: organizationId, governorId, visit_type, title, scheduled_date' },
                { status: 400 }
            );
        }
...
export async function PATCH(req: NextRequest) {
    try {
        ...
        const { organizationId, updates } = body as {
            ...
            updates: Array<{
                id: string;
                changes: Partial<GovernorVisitForm> & { status?: VisitStatus; rating?: string; findings?: string; recommendations?: string[] };
            }>;
        };
        ...
        console.error('Bulk visit update error:', error);
        return NextResponse.json(
            { error: error.message || 'Internal server error' },
            { status: 500 }
        );
