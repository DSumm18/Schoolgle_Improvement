...
export async function GET(req: NextRequest) {
    try {
        ...
        const upcoming = meetings?.filter((m: GovernorMeeting) => m.scheduled_date >= today).length || 0;
        const past = (meetings?.length || 0) - upcoming;

        // Get board info for governor names
        const governorIds = meetings?.flatMap((m: GovernorMeeting) => [
            ...m.invited_governors,
        ...
        const enrichedMeetings = meetings?.map((m: any) => ({
            ...m,
            invited_governors_details: m.invited_governors?.map((id: string) => governorsMap[id]) || [],
            attended_governors_details: m.attended_governors?.map((id: string) => governorsMap[id]) || [],
        })) || [];
...
export async function POST(req: NextRequest) {
    try {
        ...
        if (!organizationId || !title || !meeting_type || !scheduled_date) {
            return NextResponse.json(
                { error: 'Missing required fields: organizationId, title, meeting_type, scheduled_date' },
                { status: 400 }
            );
        }
...
export async function PATCH(req: NextRequest) {
    try {
        ...
        const results = await Promise.all(
            updates.map(async ({ id, changes }) => {
                ...
                if (changes.agenda_items) {
                    updateData.agenda_items = changes.agenda_items.map((item: any, index: number) => ({
                        id: item.id || uuidv4(),
                        title: item.title,
                        description: item.description || '',
                        owner: item.owner || '',
        ...
        console.error('Bulk meeting update error:', error);
        return NextResponse.json(
            { error: error.message || 'Internal server error' },
            { status: 500 }
        );
