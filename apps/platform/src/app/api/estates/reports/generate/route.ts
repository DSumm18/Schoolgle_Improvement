...
import {
  RAGStatusService,
  type RAGReport,
} from "@/lib/estates-compliance/services/RAGStatusService";
...
async function generateGovernorReport(
  ...
  // Get RAG status data
  const ragReport: RAGReport =
    await RAGStatusService.calculateOverallStatus(organizationId);

  // Get domain summaries
  const domains = await generateDomainSummaries(organizationId, includeDomains);

  // Get actions required
  const actionsRequired = await generateActionsRequired(
    organizationId,
    classification,
  );

  // Get budget requirements
  const budgetRequirements = await generateBudgetSummary(
    organizationId,
    classification,
  );

  // Determine if ready for Ofsted
  const readyForOfsted =
    ragReport.overall === "green" && ragReport.overallScore >= 90;
...
async function generateDomainSummaries(
  ...
  for (const domain of domainsToInclude) {
    ...
    const goodPracticeChecks = checks.filter(
      (c) => c.category === "good_practice",
    );
...
async function generateActionsRequired(
  ...
  const actions: ActionRequired[] = overdueTasks.map((task) => ({
    ...
    statutoryReference: STATUTORY_CHECKS[
      task.compliance_domain as ComplianceDomain
    ]?.find((c) => c.id === task.task_type)?.reference,
  }));
...
async function generateEvidenceSummary(organizationId: string) {
  ...
  return {
    ...
    evidenceByDomain: domains.reduce(
      (acc, domain) => {
...
function generateExecutiveSummary(
  ...
  const statutoryChecksTotal = domains.reduce(
    (sum, d) => sum + d.statutoryChecksTotal,
    0,
  );
  const statutoryChecksCompleted = domains.reduce(
    (sum, d) => sum + d.statutoryChecksCompleted,
    0,
  );
  const statutoryComplianceRate =
    statutoryChecksTotal > 0
  ...
  return {
    ...
    headlineStatement: getHeadlineStatement(
      ragReport.overall,
      ragReport.overallScore,
    ),
    keyAchievements: getKeyAchievements(domains),
    criticalIssues,
    immediateActionsRequired: domains.reduce(
...
function getKeyAchievements(domains: DomainSummary[]): string[] {
  ...
  domains.forEach((domain) => {
    if (
      domain.ragStatus === "green" &&
      domain.statutoryChecksCompleted === domain.statutoryChecksTotal
    ) {
      achievements.push(
        `${domain.domainName}: All statutory checks completed and compliant`,
      );
    }
    if (domain.completionRate >= 90) {
      achievements.push(
        `${domain.domainName}: ${domain.completionRate}% compliance rate achieved`,
      );
    }
  });
...
function getOfstedReadinessDetails(
  ready: boolean,
  domains: DomainSummary[],
): string[] {
  if (ready) {
  ...
  if (redDomains.length > 0) {
    details.push(
      `${redDomains.length} domain(s) require immediate attention before inspection`,
    );
  }
  if (amberDomains.length > 0) {
    details.push(
      `${amberDomains.length} domain(s) have areas that may be queried`,
    );
  }
...
function generateRecommendations(
  ...
  if (executiveSummary.readyForOfsted) {
    forGovernors.push(
      "Maintain current compliance processes to ensure continued readiness",
    );
  } else {
    forGovernors.push(
      "Review critical compliance issues at next governing board meeting",
    );
    forGovernors.push(
      "Ensure adequate budget allocation for statutory compliance items",
    );
  }
  ...
  if (domainsWithIssues.length > 0) {
    forSeniorLeadership.push(
      "Prioritize completion of overdue compliance checks",
    );
    forSeniorLeadership.push(
      "Review staff responsibilities for compliance monitoring",
    );
  }

  // Business manager recommendations
  forBusinessManager.push(
    "Review statutory budget requirements for next fiscal year",
  );
  forBusinessManager.push(
    "Ensure contracts for statutory services are up to date",
  );
  forBusinessManager.push(
    "Schedule annual inspections and renewals in advance",
  );
