...
const MODEL_SELECT =
  "id, organization_id, name, status, source_document_url, source_document_name, extraction_result, metrics, approved_at, published_at, created_at, updated_at";
...
function toStatus(value: unknown): PathfinderModelStatus {
  return STATUS_VALUES.includes(value as PathfinderModelStatus) ? (value as PathfinderModelStatus) : "school_review";
}
...
export const POST = protectedRoute(
  async (auth, request) => {
    ...
    const row = {
      ...
      name: typeof body.name === "string" && body.name.trim() ? body.name.trim() : extractionResult.image.title,
      status,
      source_document_url: typeof body.sourceDocumentUrl === "string" ? body.sourceDocumentUrl : null,
      source_document_name: typeof body.sourceDocumentName === "string" ? body.sourceDocumentName : extractionResult.image.title,
