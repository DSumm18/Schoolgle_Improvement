...
export default function ActionsHubPage() {
    ...
    try {
      ...
      const mappedActions: Action[] = (data || []).map((a: any) => ({
        ...
        implementation_date: a.implementation_date
          ? new Date(a.implementation_date)
          : undefined,
  ...
  function calculateStats(actions: Action[]) {
    const completed = actions.filter(
      (a) => a.user_status === "complete",
    ).length;
    const inProgress = actions.filter(
      (a) => a.user_status === "in_progress",
    ).length;
    const gaps = actions.filter(
      (a) => a.ai_status === "not_met" && a.user_status !== "complete",
    ).length;
    const totalEstimated = actions.reduce(
      (sum, a) => sum + (a.estimated_cost || 0),
      0,
    );
    const totalActual = actions.reduce(
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          progress:
            action.user_status === "complete"
              ? 100
              : action.user_status === "in_progress"
                ? 50
                : 0,
        })
        .eq("id", action.id);
  ...
  return (
    <div className="p-8 space-y-8 max-w-[1800px] mx-auto min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-4">
        ...
        >
          <div className="flex items-center gap-3 text-sky-500 font-black text-xs uppercase tracking-[0.2em] mb-2 bg-sky-50 w-fit px-3 py-1 rounded-full border border-sky-100">
      ...
      <motion.div
        ...
        className="bg-white rounded-2xl p-1 border border-slate-200 shadow-lg relative"
      >
        <div className="p-8">
          <ActionsDashboard
            actions={actions}
