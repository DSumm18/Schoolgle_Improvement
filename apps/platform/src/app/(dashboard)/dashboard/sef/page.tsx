...
import {
  SEFGenerator,
  SEFSectionData,
  SEFGenerationOptions,
  ToneStyle,
} from "@/lib/sef-generator";
import dynamic from "next/dynamic";
import ErrorBoundary from "@/components/common/ErrorBoundary";

const SEFSectionEditor = dynamic(
  () => import("@/components/sef/SEFSectionEditor"),
  {
    loading: () => (
...
const SEFVersionHistory = dynamic(
  () => import("@/components/sef/SEFVersionHistory"),
  {
    loading: () => (
      <div className="flex-1 bg-slate-50 dark:bg-slate-800/50 animate-pulse rounded-2xl" />
    ),
...
export default function SEFPage() {
  ...
  const [activeTab, setActiveTab] = useState<"editor" | "preview" | "history">(
    "editor",
  );
  ...
  const handleGenerateSection = async (sectionId: string) => {
    ...
    try {
      ...
      setError(
        err.message ||
          "Failed to generate section. Please check your internet connection.",
      );
    } finally {
  ...
  const handleSave = async (isNewVersion: boolean = true) => {
    ...
    try {
      ...
      alert(
        isNewVersion ? "New version archived" : "Draft updated successfully",
      );
    } catch (err) {
      console.error(err);
  ...
  const handlePublish = async () => {
    if (
      !confirm(
        "Are you sure you want to publish this SEF? This will lock it from further editing.",
      )
    )
      return;
      ...
      <div className="p-8 max-w-[1700px] mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <div className="lg:col-span-3 space-y-6">
            <div className="bg-white dark:bg-slate-900/50 backdrop-blur-3xl rounded-2xl p-6 border border-slate-100 dark:border-slate-800 shadow-md">
                ...
                {SEF_AREAS.map((area) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`p-2.5 rounded-xl bg-white dark:bg-slate-800 shadow-sm ${activeSection === area.id ? "text-blue-600 ring-2 ring-blue-100" : "text-slate-400"}`}
                      >
                        <area.icon size={20} />
                      </div>
                      <div className="text-left">
                        ...
                        {sections[area.id] && (
                          <span className="text-[9px] text-emerald-500 font-bold uppercase mt-1 block">
                            Completed
                          </span>
                        )}
                      </div>
            ...
            <div className="bg-gradient-to-br from-slate-900 to-slate-950 rounded-2xl p-8 text-white shadow-lg border border-white/5">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-[9px] font-black uppercase text-slate-500 mb-2 block">
                    Voice Tone
                  </label>
                  <select
                    value={options.tone}
          ...
          <div className="lg:col-span-9">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="bg-white dark:bg-slate-900/50 backdrop-blur-3xl rounded-2xl p-10 border border-slate-200 dark:border-slate-800 shadow-lg min-h-[750px] flex flex-col"
              >
                {activeTab === "editor" ? (
                  sections[activeSection] ? (
                    <SEFSectionEditor
                      ...
                      onChange={(updated) =>
                        setSections({ ...sections, [activeSection]: updated })
                      }
                    />
                  ) : (
                    <div className="flex-1 flex flex-col items-center justify-center text-center space-y-8">
                      <div className="w-32 h-32 bg-blue-50 dark:bg-blue-900/20 rounded-2xl flex items-center justify-center text-sky-600 relative overflow-hidden group">
                      ...
                      <div className="max-w-md">
                        ...
                        <p className="text-slate-500 font-bold mt-3 leading-relaxed">
                          ...
                          <strong>
                            {
                              SEF_AREAS.find((a) => a.id === activeSection)
                                ?.title
                            }
                          </strong>{" "}
                          based on your evidence base and internal assessments.
                        </p>
                      </div>
                      <button
                        onClick={() => handleGenerateSection(activeSection)}
                        disabled={!!isGenerating}
                        className="group flex items-center gap-4 px-12 py-5 bg-blue-600 text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] hover:bg-blue-700 transition-all shadow-[0_20px_40px_rgba(37,99,235,0.3)] hover:scale-105 active:scale-95 disabled:opacity-50"
                      >
                    ...
                    <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-800/50 p-6 rounded-2xl border border-slate-100 dark:border-slate-800">
                      <div>
                        ...
                        <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-widest">
                          Optimized for PDF & Web Export
                        </p>
                      </div>
                      <div className="flex gap-4">
                        <button
                          onClick={handleExportPDF}
                          className="px-6 py-3 bg-white dark:bg-slate-700 text-blue-600 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 border border-blue-50 dark:border-blue-900 shadow-sm hover:shadow-md transition-all"
                        >
                          <Download size={16} /> Export HTML
                        </button>
                        <button
                          onClick={handleExportPDF}
                          className="px-6 py-3 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-lg hover:bg-blue-700 transition-all"
                        >
                          <Copy size={16} /> Print/PDF
                        </button>
                      </div>
                    </div>
                    <div className="flex-1 border-4 border-slate-100 dark:border-slate-800 rounded-2xl overflow-hidden bg-white shadow-inner">
                      <iframe
                        title="SEF Preview"
                        srcDoc={SEFGenerator.exportToHTML({
                          academicYear: options.academicYear,
                          sections: Object.values(sections),
                        })}
        ...
        {/* Advisory Footer */}
        <div className="bg-gradient-to-br from-sky-600 to-sky-700 rounded-2xl p-10 mt-12 flex items-center justify-between border border-white/10 shadow-lg relative overflow-hidden group">
          ...
          <div className="flex items-center gap-8 relative z-10">
            ...
            <div>
              <h4 className="text-2xl font-black text-white tracking-tight">
                Inspection Readiness Guard
              </h4>
              <p className="text-white/70 font-bold text-sm mt-1">
                Ed has verified that all 4 areas have substantive evidence
                backing. Your SEF is considered 'Inspection Ready'.
              </p>
            </div>
          </div>
          <div className="flex gap-4 relative z-10">
            <button className="px-8 py-4 bg-white/10 hover:bg-white/20 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border border-white/10 backdrop-blur-lg hover:scale-105">
              Audit Trace
            </button>
            <button
              onClick={handlePublish}
              disabled={isPublishing || currentDoc?.status === "published"}
              className="px-10 py-4 bg-white text-sky-900 rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all shadow-[0_20px_40px_rgba(0,0,0,0.2)] hover:scale-105 active:scale-95 disabled:bg-slate-200"
            >
