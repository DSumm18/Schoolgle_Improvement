...
export default function ActionPlanPage() {
  ...
  async function fetchActions() {
    ...
    try {
      ...
      const completed = mappedActions.filter(
      ...
      setStats((prev) => ({
        ...
        completedPercentage:
          mappedActions.length > 0
            ? Math.round((completed / mappedActions.length) * 100)
            : 0,
        highPriorityGaps: mappedActions.filter(
          (a) =>
            (a.priority === "high" || (a as any).priority === "critical") &&
            a.status !== "completed",
        ).length,
      }));
    } catch (err) {
    ...
    try {
      ...
      if (assessments && assessments.length > 0) {
        const total = assessments.reduce(
          (acc, curr) => acc + (curr.evidence_count > 0 ? 1 : 0),
          0,
        );
  ...
  const handleUpdateAction = async (action: ActionItem) => {
    try {
      ...
      if (
        action.assigneeId &&
        (!oldAction || oldAction.assigneeId !== action.assigneeId)
      ) {
        NotificationService.send({
          organizationId: organization?.id!,
  ...
  return (
    <div className="p-8 space-y-8 max-w-[1600px] mx-auto min-h-screen">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-4">
        ...
        >
          <div className="flex items-center gap-3 text-sky-500 font-black text-xs uppercase tracking-[0.2em] mb-2 bg-sky-50 dark:bg-sky-900/20 w-fit px-3 py-1 rounded-full border border-sky-100 dark:border-sky-800">
        ...
        <div className="flex gap-4">
          <button
            ...
            className="flex items-center gap-2.5 px-6 py-3.5 bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-2xl text-sm font-black text-slate-700 dark:text-slate-200 hover:bg-slate-50 transition-all shadow-sm hover:shadow-xl active:scale-95 group"
          >
            <Download
              size={20}
          ...
          <button
            ...
            className="flex items-center gap-2.5 px-8 py-3.5 bg-sky-600 rounded-2xl text-sm font-black text-white hover:bg-sky-700 transition-all shadow-[0_10px_30px_rgba(37,99,235,0.3)] hover:shadow-[0_15px_40px_rgba(37,99,235,0.4)] hover:scale-105 active:scale-95"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatsCard
          ...
          icon={
            <Brain size={24} className="text-purple-500 dark:text-purple-400" />
          }
          color="purple"
      ...
      <motion.div
        ...
        className="bg-white dark:bg-slate-900/50 backdrop-blur-3xl rounded-2xl p-1 border border-slate-200 dark:border-slate-800 shadow-lg relative"
      >
        <div className="p-8">
          <ActionsDashboard
            actions={actions}
      ...
      <div className="bg-gradient-to-br from-sky-600 to-sky-700 rounded-2xl p-8 flex flex-col md:flex-row items-center gap-8 shadow-lg relative overflow-hidden group">
        ...
        <div className="p-5 bg-white shadow-lg rounded-2xl text-sky-600 relative z-10 shrink-0">
        ...
        <div className="relative z-10 text-center md:text-left flex-1">
          <h4 className="font-black text-2xl text-white tracking-tight">
            Ed AI Strategy Recommendation
          </h4>
          <p className="text-blue-100 font-medium text-lg mt-2 max-w-4xl leading-relaxed">
            Based on your current evidence scan, completing the{" "}
            <strong>"Impact analysis of Pupil Premium tutoring"</strong> will
            generate the highest readiness lift (<strong>+6.4%</strong>) for
            your Leadership & Management judgement area.
          </p>
        </div>

        <button className="px-8 py-4 bg-white text-sky-700 font-black text-sm uppercase tracking-widest rounded-2xl hover:bg-sky-50 hover:scale-105 active:scale-95 transition-all shadow-md shrink-0 relative z-10">
...
function StatsCard({
  ...
  return (
    <motion.div
      ...
      className={`p-8 rounded-2xl border ${colors[color]} shadow-md backdrop-blur-xl relative overflow-hidden group transition-all duration-300`}
    >
      ...
      <div className="space-y-1 relative z-10 mt-4">
        <h3 className="text-slate-400 dark:text-slate-500 text-[10px] font-black uppercase tracking-[0.15em]">
          {label}
        </h3>
      ...
      <div
        className={`absolute -bottom-10 -right-10 w-32 h-32 blur-3xl opacity-0 group-hover:opacity-30 transition-opacity duration-500 ${color === "blue" ? "bg-blue-400" : color === "rose" ? "bg-rose-400" : color === "emerald" ? "bg-emerald-400" : "bg-purple-400"}`}
      />
    </motion.div>
  );
}
