...
export default function ApprovalHubPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      setHasCriticalIncident(
        mockItems.some((i) => i.skill_id.includes("lockdown")),
      );
      setLoading(false);
    }, 1000);
  ...
  const handleApprove = (id: string) => {
    ...
    setHasCriticalIncident(
      remaining.some((i) => i.skill_id.includes("lockdown")),
    );
    toast.success("Message Approved", {
      description:
        "Ed has dispatched the communication via the requested channel.",
    });
  };
  ...
  const handleDeny = (id: string) => {
    ...
    toast.error("Message Rejected", {
      description:
        "The communication has been cancelled and logged as rejected.",
    });
  };

  return (
    <div
      className={`flex-1 space-y-6 p-8 pt-6 min-h-screen transition-colors duration-500 ${hasCriticalIncident ? "bg-red-950/20" : "bg-neutral-950"} text-neutral-50 overflow-auto`}
    >
      {hasCriticalIncident && (
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2
            className={`text-3xl font-bold tracking-tight ${hasCriticalIncident ? "text-red-500 animate-pulse" : "bg-gradient-to-r from-sky-400 to-blue-400 bg-clip-text text-transparent"}`}
          >
            {hasCriticalIncident
        ...
        <Card className="bg-neutral-900/50 border-neutral-800 backdrop-blur-xl">
          ...
          <CardContent>
            ...
            <Progress
              ...
              className="h-1 mt-3 bg-neutral-800"
            />
          </CardContent>
        </Card>
        <Card className="bg-neutral-900/50 border-neutral-800 backdrop-blur-xl text-sky-400">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-neutral-400">
              Intervention Rate
            </CardTitle>
      ...
      <div className="grid gap-6 md:grid-cols-7">
        ...
        <div className="md:col-span-4 space-y-4">
          <div className="flex items-center justify-between bg-neutral-900/80 p-4 rounded-xl border border-neutral-800 shadow-xl">
            ...
            <Badge
              variant="secondary"
              className="bg-sky-900/30 text-sky-400 border-sky-500/30"
            >
          ...
          {loading ? (
            ...
            pendingItems.map((item) => {
              const isCritical =
                item.skill_id.includes("lockdown") ||
                item.skill_id.includes("emergency");
              return (
                <Card
                  key={item.id}
                  className={`relative group overflow-hidden bg-neutral-900/60 border-neutral-800 transition-all duration-300 backdrop-blur-md shadow-lg ${
                    isCritical
                ...
                >
                  <div className="absolute top-0 right-0 p-4 flex gap-2">
                    {isCritical && (
                      <Badge
                        variant="destructive"
                        className="animate-pulse bg-red-600 text-white border-transparent"
                      >
                        CRITICAL
                      </Badge>
                    )}
                    <Badge
                      className={`${isCritical ? "bg-red-900/40 text-red-200 border-red-500/30" : "bg-neutral-800/80 text-neutral-300 border-neutral-700"} capitalize`}
                    >
                  ...
                  <CardHeader>
                    ...
                    <CardDescription className="flex items-center gap-2 text-neutral-500">
                      ...
                      Confidence:
                      <span
                        className={`font-bold ${item.confidence > 0.9 ? "text-green-500" : "text-yellow-500"}`}
                      >
                  ...
                  <CardFooter className="flex gap-2 pt-0">
                    <Button
                      className="flex-1 bg-sky-600 hover:bg-sky-500 text-white font-semibold transition-all shadow-md active:scale-95"
                    ...
                    <Button
                      variant="outline"
                      className="flex-1 border-neutral-700 text-neutral-300 hover:bg-neutral-800 hover:text-white transition-all active:scale-95"
                    >
                      Edit
                    </Button>
                    <Button
        ...
        <div className="md:col-span-3 space-y-6">
          <Card className="bg-neutral-900/80 border-neutral-800 shadow-xl overflow-hidden">
            ...
            <CardContent className="space-y-4 p-4">
              {[
                {
                  tier: "Tier 1: AUTO",
                  desc: "No intervention. Standard usage.",
                  active: true,
                },
                {
                  tier: "Tier 2: SHADOW",
                  desc: "Ed logs but sends copies for review.",
                  active: false,
                },
                {
                  tier: "Tier 3: REVIEW",
                  desc: "Human must approve every word.",
                  active: true,
                },
                {
                  tier: "Tier 4: BLOCKED",
                  desc: "Killswitch. No comms allowed.",
                  active: false,
                },
              ].map((t, i) => (
                <div
                  key={i}
                  className={`p-3 rounded-lg border transition-all ${t.active ? "bg-sky-950/20 border-sky-500/30" : "bg-neutral-950/30 border-neutral-800 opacity-60"}`}
                >
                  <div className="flex items-center justify-between mb-1">
                    <span
                      className={`text-xs font-bold ${t.active ? "text-sky-400" : "text-neutral-500"}`}
                    >
                      {t.tier}
                    </span>
                    <div
                      className={`w-8 h-4 rounded-full p-1 ${t.active ? "bg-sky-600" : "bg-neutral-800 flex justify-end"}`}
                    >
          ...
          <Card className="bg-neutral-900/80 border-neutral-800 shadow-xl overflow-hidden flex flex-col h-[350px]">
            ...
            <CardContent className="p-0 overflow-auto divide-y divide-neutral-800 shrink-1 h-full">
              {[
                {
                  school: "St. Judes Primary",
                  skill: "Staff Absence",
                  time: "10m ago",
                },
                {
                  school: "St. Judes Primary",
                  skill: "Contractor Chase",
                  time: "2h ago",
                },
                {
                  school: "Schoolgle Empire",
                  skill: "DBS Update",
                  time: "5h ago",
                },
                {
                  school: "St. Judes Primary",
                  skill: "Compliance Alert",
                  time: "Yesterday",
                },
              ].map((log, i) => (
                <div
                  key={i}
                  className="p-3 hover:bg-neutral-800/30 transition-colors flex items-center justify-between"
                >
                  <div>
