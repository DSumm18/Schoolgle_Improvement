...
const InsightCard = ({ insight, index, scrollProgress }: any) => {
  ...
  return (
    <motion.article style={{ scale, opacity, y }} className="group">
      <Link
        href={`/insights/${insight.slug}`}
        className="block h-full bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-700/50 rounded-2xl overflow-hidden hover:border-lp-accent/50 transition-all duration-500 hover:-translate-y-2 group"
      >
        {insight.heroImage && (
        ...
        <div className="p-10">
          <div className="flex items-start justify-between mb-6">
            ...
            <ArrowRight
              size={20}
              className="text-lp-text-muted group-hover:text-lp-accent group-hover:translate-x-1 transition-all"
            />
          </div>
...
export default function InsightsPage() {
  ...
  return (
    <div
      ref={containerRef}
      className="min-h-[250vh] bg-lp-bg relative selection:bg-lp-accent selection:text-white"
    >
      <Navbar />
        ...
        >
          <div className="max-w-7xl mx-auto text-center">
            ...
            >
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-lp-text-sec">
            ...
            <p className="text-2xl md:text-3xl text-lp-text-sec font-medium max-w-3xl mx-auto leading-relaxed">
              ...
              <span className="text-lp-text">school leadership</span> and{" "}
              <span className="text-lp-text">emerging intelligence</span>.
            </p>
          </div>
        </motion.section>
              ...
              <div className="mt-48 text-center">
                ...
                >
                  <h2 className="text-4xl md:text-5xl font-black text-lp-text outfit uppercase tracking-tighter">
                    The roadmap <br />{" "}
                    <span className="text-lp-accent italic">Ahead.</span>
                  </h2>
                </motion.div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
                  {comingSoonInsights.map((insight, index) => (
                    <motion.div
                      ...
                      className="p-8 bg-white dark:bg-slate-900 border border-dashed border-lp-border grayscale opacity-50 hover:grayscale-0 hover:opacity-100 transition-all duration-700 rounded-2xl text-left"
                    >
                      <span className="text-[10px] font-black uppercase tracking-widest text-lp-accent mb-4 block">
                        Coming Soon
                      </span>
                      <h3 className="text-2xl font-black text-lp-text outfit uppercase tracking-tighter mb-4">
                        {insight.title}
                      </h3>
                      <p className="text-lp-text-sec font-medium leading-relaxed">
                        {insight.excerpt}
                      </p>
                    </motion.div>
                  ))}
