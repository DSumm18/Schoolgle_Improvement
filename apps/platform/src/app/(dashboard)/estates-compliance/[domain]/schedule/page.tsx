...
export default function DomainSchedulePage() {
  ...
  const handleUpdateSchedule = (checkId: string, updates: Partial<ScheduledCheck>) => {
    setSchedules((prev) => ({
      ...prev,
  ...
  return (
    <div className="space-y-6 p-6">
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            <Link
              ...
              className="inline-flex items-center gap-1 text-sm font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300"
            >
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Configure automated reminders and task scheduling for {metadata.name}
          </p>
        </div>
        <button
          onClick={handleSave}
          disabled={saving}
          className="inline-flex items-center gap-2 rounded-lg border-2 border-indigo-600 dark:border-indigo-500 bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-400 disabled:border-gray-400 px-6 py-2.5 text-sm font-bold text-white shadow-md hover:shadow-lg transition-all disabled:cursor-not-allowed"
        >
          {saved ? (
            <>
      ...
      {/* Schedule Overview */}
      <div className="rounded-xl border-2 border-indigo-300 dark:border-indigo-700 bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 dark:from-blue-950/30 dark:via-indigo-950/30 dark:to-purple-950/30 p-6 shadow-lg">
        <div className="flex items-center gap-3 mb-4">
          <div className="p-2 rounded-lg bg-indigo-100 dark:bg-indigo-900/50">
            <Calendar className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
          </div>
          <div>
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">Schedule Overview</h2>
        ...
        <div className="flex flex-wrap gap-3">
          <div className="px-4 py-2 rounded-lg bg-white dark:bg-gray-800 border-2 border-gray-300 dark:border-gray-600">
            ...
            <div className="text-xs text-gray-600 dark:text-gray-400 font-semibold">Auto-Enabled</div>
          </div>
          <div className="px-4 py-2 rounded-lg bg-white dark:bg-gray-800 border-2 border-gray-300 dark:border-gray-600">
            <div className="text-2xl font-bold text-green-600 dark:text-green-400">
              {Object.values(schedules).filter((s) => s.reminderDays > 0).length}
            </div>
            <div className="text-xs text-gray-600 dark:text-gray-400 font-semibold">With Reminders</div>
          </div>
        </div>
      </div>
              ...
              <div className="p-6">
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap mb-2">
                      <h3 className="font-bold text-lg text-gray-900 dark:text-white">{check.name}</h3>
                      <span
                  ...
                  <div className="text-right bg-gray-50 dark:bg-gray-900/50 rounded-lg px-4 py-2 border-2 border-gray-200 dark:border-gray-700">
                    <div className="text-xs text-gray-600 dark:text-gray-400 font-semibold mb-1">Next Due</div>
                    <div className="text-sm font-bold text-indigo-600 dark:text-indigo-400">{nextDue}</div>
                  </div>
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleUpdateSchedule(check.id, {
                          frequency: e.target.value as ScheduledCheck['frequency'],
                        })
                      }
                      className="w-full px-3 py-2 rounded-lg border-2 border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm font-semibold focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => handleUpdateSchedule(check.id, { startDate: e.target.value })}
                      className="w-full px-3 py-2 rounded-lg border-2 border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm font-semibold focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleUpdateSchedule(check.id, { reminderDays: parseInt(e.target.value) || 0 })
                      }
                      className="w-full px-3 py-2 rounded-lg border-2 border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm font-semibold focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                    />
                  </div>
                    ...
                    <div className="h-10 flex items-center">
                      <label className="relative inline-flex items-center cursor-pointer">
                        <input
                          ...
                          onChange={(e) => handleUpdateSchedule(check.id, { autoCreate: e.target.checked })}
                          className="sr-only peer"
                        />
                        <div className="w-14 h-7 bg-gray-300 dark:bg-gray-600 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-indigo-300 dark:peer-focus:ring-indigo-800 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-0.5 after:left-[4px] after:bg-white after:rounded-full after:h-6 after:w-6 after:transition-all peer-checked:bg-indigo-600 border-2 border-gray-400"></div>
                      </label>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-4 pt-4 border-t-2 border-gray-200 dark:border-gray-700">
                  ...
                  <input
                    ...
                    onChange={(e) => handleUpdateSchedule(check.id, { assignee: e.target.value || undefined })}
                    className="w-full sm:w-64 px-3 py-2 rounded-lg border-2 border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm font-semibold focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                  />
                </div>
              </div>
            </div>
      ...
      <div className="rounded-xl border-2 border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50 p-6">
        <h3 className="font-bold text-lg text-gray-900 dark:text-white mb-4">Bulk Actions</h3>
        <div className="flex flex-wrap gap-3">
          <button
            onClick={() => {
