...
export default function CheckHistoryPage() {
  ...
  const [filter, setFilter] = useState<'all' | 'completed' | 'awaiting_documentation'>('all');
  ...
  useEffect(() => {
    ...
    async function initializeData() {
      setDebugInfo({ step: 'Starting...', checkId: checkId || 'none', domain: domainSlug || 'none', count: 0 });
      ...
      if (mounted) {
        setCheck(foundCheck);
        setDebugInfo(prev => ({ ...prev, step: 'Check found: ' + foundCheck.name }));
      }
      ...
      if (organizationId) {
        ...
        try {
          console.log('[HISTORY] Fetching completions for:', { organizationId, checkId, domainSlug });
          ...
          const checkCompletions = data.filter((r: any) => r.check_id === checkId);
          ...
          if (mounted) {
            ...
            const historyRecords: HistoryRecord[] = checkCompletions.map((record: any) => ({
              id: record.id,
            ...
            historyRecords.sort((a, b) => new Date(b.completedDate).getTime() - new Date(a.completedDate).getTime());

            setHistory(historyRecords);
            setDebugInfo(prev => ({ ...prev, step: 'Loaded ' + historyRecords.length + ' records', count: historyRecords.length }));
            setLoading(false);
          }
        } catch (error: any) {
          clearTimeout(timeoutId);
          if (mounted) {
            if (error.name === 'AbortError' || error?.message?.includes('aborted')) {
              ...
              setDebugInfo(prev => ({ ...prev, step: 'ERROR: ' + String(error) }));
            }
        ...
        if (mounted) {
          setDebugInfo(prev => ({ ...prev, step: 'No organizationId - showing empty' }));
          setLoading(false);
        }
      }
    }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4">
        ...
        <div className="text-gray-600 dark:text-gray-400 font-semibold text-lg">Loading history...</div>
      </div>
    );
  }
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col gap-4 pb-6 border-b-2 border-gray-200 dark:border-gray-700">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">Compliance History</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="rounded-xl border-2 border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 p-5 shadow-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-gray-600 dark:text-gray-400">Total Completions</p>
        ...
        <div className="rounded-xl border-2 border-green-300 dark:border-green-700 bg-green-50 dark:bg-green-950/30 p-5 shadow-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-green-700 dark:text-green-300">Fully Complete</p>
        ...
        <div className="rounded-xl border-2 border-amber-300 dark:border-amber-700 bg-amber-50 dark:bg-amber-950/30 p-5 shadow-lg">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-amber-700 dark:text-amber-300">Awaiting Docs</p>
              <p className="text-3xl font-bold text-amber-700 dark:text-amber-300">
                {history.filter((h) => h.status === 'awaiting_documentation').length}
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex gap-2">
            {[
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-semibold border-2 transition-all ${filter === f.value
          ...
          {filteredHistory.map((record, idx) => {
            ...
            return (
              <div key={record.id} className="relative flex gap-6">
                {/* Timeline Dot */}
                <div className={`relative z-10 w-16 h-16 rounded-full border-4 flex items-center justify-center shadow-lg ${statusInfo.bg} ${statusInfo.border}`}>
                  {statusInfo.icon}
                </div>

                {/* Content */}
                <div className={`flex-1 rounded-xl border-2 shadow-lg overflow-hidden ${statusInfo.bg} ${statusInfo.border}`}>
                  <div className="p-6">
                    {/* Header */}
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
                      <div>
                        <div className="flex items-center gap-3 mb-2">
                          <h3 className="text-xl font-bold text-gray-900 dark:text-white">
                            {idx === 0 ? 'Most Recent' : `Completion #${history.length - idx}`}
                          </h3>
                          <span className={`px-3 py-1 rounded-lg text-sm font-bold border-2 flex items-center gap-1.5 shadow-sm ${statusInfo.text} ${statusInfo.border} bg-white dark:bg-gray-800`}>
                            {statusInfo.icon}
                            {statusInfo.label}
                          </span>
                        </div>
                      ...
                      {record.contractorName && (
                        <div className="px-4 py-2 rounded-lg bg-white dark:bg-gray-800 border-2 border-gray-300 dark:border-gray-600">
                          <p className="text-xs font-semibold text-gray-600 dark:text-gray-400">Contractor</p>
                          <p className="font-bold text-gray-900 dark:text-white">{record.contractorName}</p>
                        </div>
                      )}
                    </div>
                    ...
                    {record.notes && (
                      <div className="mb-4 p-4 rounded-xl bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700">
                        <p className="text-gray-800 dark:text-gray-200 leading-relaxed">{record.notes}</p>
                      </div>
                    )}
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-gray-500 dark:text-gray-400">
                                  {ev.uploadedBy} • {ev.fileSize || new Date(ev.uploadedAt).toLocaleDateString('en-GB')}
                                </p>
                              </div>
                    ...
                    <div className="mt-4 flex flex-wrap gap-3">
                      <div className={`flex items-center gap-2 px-4 py-2 rounded-lg border-2 text-sm font-bold ${record.documentsReceived
                        ...
                        {record.documentsReceived ? <Check className="w-4 h-4" /> : <AlertTriangle className="w-4 h-4" />}
                        {record.documentsReceived ? 'Documents Received' : 'Documents Outstanding'}
                      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-purple-600 text-white text-xs py-2 px-4 font-mono z-50">
        <div className="flex flex-wrap items-center justify-between gap-1">
          ...
          <span><strong>Domain:</strong> {debugInfo.domain}</span>
          <span><strong>CheckID:</strong> {debugInfo.checkId}</span>
          <span><strong>Records:</strong> {debugInfo.count}</span>
        </div>
      </div>
    </div>
  );
