...
export default function EvidenceLibraryPage() {
  ...
  const fetchEvidence = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/estates/evidence?${params.toString()}`,
        {
          headers: {
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch(
        `/api/estates/evidence/stats?organizationId=${organizationId}`,
        {
          headers: {
            Authorization: `Bearer ${session?.access_token}`,
          },
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {hasEvidence
              ? `${evidence.length} item${evidence.length !== 1 ? "s" : ""} in library`
      ...
      <div className="rounded-lg border bg-card p-4">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-6">
          <select
            ...
            onChange={(e) =>
              handleFilterChange("evidence_type", e.target.value)
            }
          >
          ...
          <select
            ...
            onChange={(e) =>
              handleFilterChange("compliance_domain", e.target.value)
            }
          >
      ...
      {hasEvidence ? (
        viewMode === "grid" ? (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {evidence.map((item) => {
              const typeInfo = evidenceTypes.find(
                (t) => t.value === item.evidence_type,
              );
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1 mb-2">
                    ...
                    <span
                      className={`text-xs rounded px-1.5 py-0.5 ${statusInfo.className}`}
                    >
                      {statusInfo.label}
                    </span>
                  </div>
        ...
        <div className="rounded-lg border border-dashed bg-card p-12 text-center">
          ...
          <p className="text-muted-foreground mb-4 max-w-sm mx-auto">
            Get started by uploading your first evidence. You can upload
            certificates, reports, photos, and more.
          </p>
          <Link
            href="/estates-compliance/evidence/upload"
            className="inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90"
