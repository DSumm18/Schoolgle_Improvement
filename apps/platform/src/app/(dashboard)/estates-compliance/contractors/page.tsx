  ...
  const fetchContractors = useCallback(
    async (signal?: AbortSignal) => {
      ...
      const timeoutId = setTimeout(
        () => controller.abort("Contractors fetch timed out"),
        30000,
      );
      ...
      try {
        ...
        console.log(
          "[ContractorsPage] Fetching:",
          `/api/estates/contractors?${params.toString()}`,
        );
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Failed to fetch contractors: ${response.status} ${response.statusText}`,
          );
        }

        const data = await response.json();
        console.log("[ContractorsPage] Received data:", {
    ...
    return [...contractors].sort((a, b) => {
      ...
      switch (sortField) {
        case "company_name":
          ...
          break;
        case "service_type":
          comparison = (a.services?.[0]?.service_type || "").localeCompare(
            b.services?.[0]?.service_type || "",
          );
          break;
        case "status":
        ...
        <>
            ...
            <div className="flex flex-wrap gap-3 items-center">
              ...
              <label className="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                <input
                  ...
                  onChange={(e) =>
                    handleFilterChange("preferred_only", e.target.checked)
                  }
                />
                Preferred only
              </label>
          ...
          {loading ? (
            ...
            <div className="border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-lg p-16 text-center bg-white dark:bg-gray-900">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-6 max-w-md mx-auto">
                Add contractors who provide compliance services to your school.
                Track their accreditations, insurance, and contracts.
              </p>
              <Link
                href="/estates-compliance/contractors/new"
                className="inline-flex items-center justify-center gap-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 text-sm font-semibold shadow-sm transition-colors"
                      ...
                      <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        {contractors.map((contractor) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3">
                              <span
                                className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-semibold border ${getStatusColor(contractor.status)}`}
                              >
                                {contractor.status}
                              </span>
                            </td>
                            <td className="px-4 py-3">
                              <div className="text-xs text-gray-600 dark:text-gray-400">
                                ...
                                contractor.accreditations.length > 0 ? (
                                  <span>
                                    {contractor.accreditations.length}{" "}
                                    accreditations
                                  </span>
                                ) : (
                            ...
                            <td className="px-4 py-3">
                              <div className="text-xs text-gray-600 dark:text-gray-400">
                                {contractor.insurance_certificates &&
                                contractor.insurance_certificates.length > 0 ? (
                                  <span>
                                    {contractor.insurance_certificates.length}{" "}
                                    policies
                                  </span>
                                ) : (
                                  <span className="text-gray-400">None</span>
