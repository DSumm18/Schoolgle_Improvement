...
import { HelpdeskTicket, TicketStatus, TicketPriority } from '@/types/estates-compliance';
import { useAuth } from '@/context/SupabaseAuthContext';
import { ArrowUpDown, Filter, Calendar, User, AlertCircle, CheckCircle, Clock, wrench } from 'lucide-react';
...
export default function HelpdeskPage() {
  ...
  const fetchTickets = async () => {
    try {
      ...
      const response = await fetch(`/api/estates/helpdesk?${params.toString()}`, {
        headers: {
  ...
  const filterTickets = (tickets: HelpdeskTicket[]) => {
    return tickets.filter(ticket => {
      ...
      if (filters.category && !ticket.category.toLowerCase().includes(filters.category.toLowerCase())) return false;
      if (filters.search) {
  ...
  return (
    <div className="space-y-6 p-6 bg-white dark:bg-gray-900 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-6 border-b border-gray-200 dark:border-gray-700">
        <div>
          <nav className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 mb-2">
            ...
            <span className="text-gray-900 dark:text-gray-100 font-medium">Helpdesk</span>
          </nav>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">Helpdesk</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1 font-medium">
            Report and track facilities issues, maintenance requests, and repairs
          </p>
        </div>
        <Link
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        <StatCard label="Total" value={stats.total} onClick={() => setActiveTab('all')} active={activeTab === 'all'} variant="default" />
        <StatCard label="Open" value={stats.open} onClick={() => setActiveTab('open')} active={activeTab === 'open'} variant="info" />
        <StatCard label="In Progress" value={stats.inProgress} onClick={() => setActiveTab('in_progress')} active={activeTab === 'in_progress'} variant="warning" />
        <StatCard label="Resolved" value={stats.resolved} onClick={() => setActiveTab('resolved')} active={activeTab === 'resolved'} variant="success" />
        <StatCard label="Closed" value={stats.closed} onClick={() => setActiveTab('closed')} active={activeTab === 'closed'} variant="default" />
      </div>
      ...
      <div className="border-b border-gray-200 dark:border-gray-700">
        <nav className="flex gap-1">
          {(['all', 'open', 'in_progress', 'resolved', 'closed'] as TabMode[]).map((tab) => (
            <button
              key={tab}
      ...
      {loading ? (
        <div className="flex items-center justify-center py-16">
          ...
          <p className="ml-3 text-gray-600 dark:text-gray-400 font-medium">Loading tickets...</p>
        </div>
      ) : error ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {tickets.map((ticket) => (
                  <tr key={ticket.id} className="text-sm hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                    <td className="px-4 py-3">
                      <div className="font-mono text-xs text-gray-500 dark:text-gray-400 mb-1">{ticket.ticket_number}</div>
                      <div className="font-semibold text-gray-900 dark:text-white">{ticket.title}</div>
                      {ticket.description && (
                        <div className="text-xs text-gray-600 dark:text-gray-400 line-clamp-1 mt-1">{ticket.description}</div>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-gray-700 dark:text-gray-300">{getCategoryLabel(ticket.category)}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-semibold border ${getPriorityColor(ticket.priority)}`}>
                        {ticket.priority}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-700 dark:text-gray-300 font-medium capitalize">{ticket.status.replace('_', ' ')}</span>
                      </div>
                    </td>
...
function StatCard({ label, value, onClick, active, variant = 'default' }: StatCardProps) {
  const variantStyles = {
    ...
    warning: 'bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-800 hover:bg-yellow-100 dark:hover:bg-yellow-900/30',
    success: 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800 hover:bg-green-100 dark:hover:bg-green-900/30',
  };
