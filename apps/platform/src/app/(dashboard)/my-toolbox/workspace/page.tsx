...
export default function ToolboxWorkspacePage() {
  ...
  useEffect(() => {
    ...
    if (edInstance && typeof edInstance.setToolContext === "function") {
      if (activeTool) {
        ...
        const expertise = TOOL_EXPERTISE[toolId] || [
          "general guidance",
          "best practices",
        ];
  ...
  const openToolInPopup = () => {
    ...
    const newWindow = window.open(
      activeTool.tool.url,
      `schoolgle_workspace_${activeTool.tool.id}`,
      windowFeatures,
    );
          ...
          {activeTool ? (
            <div className="text-center max-w-md">
              ...
              <p className="text-sm text-emerald-400 mb-2">
                {activeTool.tool.category}
              </p>
              <p className="text-gray-400 mb-6">{activeTool.tool.summary}</p>
    ...
    >
      ...
      {!collapsed && (
        <div className="flex-1 text-left min-w-0">
          <p
            className={`text-sm font-medium truncate ${isActive ? "text-emerald-400" : "text-gray-300"}`}
          >
            {tool.name}
          </p>
