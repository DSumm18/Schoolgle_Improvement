      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Centralized store for all inspection readiness evidence.
          </p>
        </div>
        <button
        ...
        <div className="bg-blue-600 text-white p-4 rounded-2xl shadow-xl flex items-center justify-between animate-in slide-in-from-bottom-5">
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className="text-blue-100 hover:text-white text-xs font-bold uppercase tracking-widest px-3 py-1 bg-blue-500 rounded-lg"
            >
              Clear Selection
            </button>
          </div>
          ...
          {evidence.map((item) => (
            ...
            >
              ...
              <div className="aspect-video bg-gray-50 relative overflow-hidden flex items-center justify-center p-4">
                ...
                <div
                  className={`absolute top-3 left-3 px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${getCategoryColor(item.category || "")}`}
                >
              ...
              <div className="p-4 flex-1 flex flex-col">
                ...
                {actions.filter((a) =>
                  a.linked_evidence?.some(
                    (le: any) => le.documentId === item.id,
                  ),
                ).length > 0 && (
                  <div className="mb-4">
                    ...
                    <div className="space-y-1">
                      {actions
                        ...
                        .map((a, i) => (
                          <div
                            key={i}
                            className="text-[10px] font-bold text-slate-500 truncate bg-slate-50 dark:bg-slate-800/50 px-2 py-1 rounded"
                          >
                            {a.description}
                          </div>
                        ))}
            ...
            <tbody className="divide-y divide-gray-50">
              {evidence.map((item) => (
                ...
                >
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-400">
                          {item.file_size_bytes
                            ? (item.file_size_bytes / 1024).toFixed(0)
                            : 0}{" "}
                          KB • {item.file_type}
                        </div>
                      </div>
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${getCategoryColor(item.category || "")}`}
                    >
                      {getCategoryLabel(item.category || "")}
                    </span>
                  </td>
