...
export default function PacksPage() {
  ...
  const fetchPacks = async () => {
    ...
    try {
      const response = await fetch(
        `/api/packs?organizationId=${organizationId}`,
      );
      if (response.ok) {
  ...
  const handleCreatePack = async (template: PackTemplate) => {
    ...
    try {
      ...
      const title = window.prompt(
        "Enter a title for this Governor Pack:",
  ...
  const getStatusBadge = (status: PackStatus) => {
    switch (status) {
      case "draft":
        return (
          <span className="text-[10px] font-black uppercase text-gray-500 bg-gray-100 px-2 py-0.5 rounded tracking-widest">
            Draft
          </span>
        );
      case "submitted":
        return (
          <span className="text-[10px] font-black uppercase text-blue-600 bg-blue-50 px-2 py-0.5 rounded tracking-widest">
            Submitted
          </span>
        );
      case "approved":
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-gray-500 mt-4 max-w-lg font-medium">
            Generate high-fidelity governor packs and statutory reports aligned
            with updated DfE and Ofsted requirements.
          </p>
        </div>
        <button
          onClick={() => setIsTemplateSelectorOpen(true)}
          className="bg-blue-600 hover:bg-blue-700 text-white p-6 rounded-2xl font-black text-lg flex items-center justify-center gap-3 shadow-md transition-all hover:shadow-lg hover:scale-105 active:scale-95 group"
        >
          <Plus size={24} />
          CREATE NEW PACK
          <ChevronRight
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { label: "Total Packs", value: packs.length, color: "blue" },
          {
            ...
            value: packs.filter((p) => p.status === "submitted").length,
            color: "amber",
          },
          {
            label: "Export Ready",
            value: packs.filter((p) => p.status === "approved").length,
            color: "emerald",
          },
        ].map((stat, i) => (
          <div
          ...
          >
            ...
            <p className={`text-4xl font-black mt-1 text-${stat.color}-600`}>
              {stat.value}
            </p>
          </div>
        ))}
        ...
        {loading ? (
          ...
          <div className="text-center py-24 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
            ...
            <p className="text-gray-500 mt-2 max-w-sm mx-auto font-medium">
              Start your first professional governor pack by choosing from our
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {packs.map((pack) => (
              <div
                ...
                className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:border-blue-100 transition-all duration-500 p-8 cursor-pointer group flex flex-col h-full"
              >
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${(pack.sections.filter((s) => s.content.length > 50).length / pack.sections.length) * 100}%`,
                        }}
                      ></div>
                    </div>
                    <span className="text-[10px] font-black text-gray-400">
                      {Math.round(
                        (pack.sections.filter((s) => s.content.length > 50)
                          .length /
                ...
                <div className="mt-auto pt-8 flex items-center justify-between border-t border-gray-50 mt-8">
                  ...
                  <div className="flex items-center gap-1 text-blue-600 font-black text-[10px] uppercase tracking-widest group-hover:translate-x-1 transition-transform">
                    {pack.status === "draft" ? "Continue" : "Open"}{" "}
