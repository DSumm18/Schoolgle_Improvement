...
export default function SEFPage() {
    ...
    const handleSave = async () => {
        ...
        try {
            const sefData = {
                ...
                overall_grade: "Good", // Simulated
                sections: Object.values(sections)
            };
            await SEFGenerator.saveVersion(organization?.id!, sefData);
            alert("Version saved to archives");
        } catch (err) {
            console.error(err);
        } finally {
            setIsSaving(false);
    ...
    const handleExportHTML = () => {
        const html = SEFGenerator.exportToHTML({ academicYear: options.academicYear, sections: Object.values(sections) });
        const blob = new Blob([html], { type: 'text/html' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `SEF_${options.academicYear.replace('/', '-')}.html`;
        a.click();
    };
            ...
            <div className="p-8 max-w-[1700px] mx-auto space-y-8">
                                ...
                                {activeTab === 'editor' ? (
                                    ...
                                    <div className="flex-1 flex flex-col space-y-6">
                                        <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-800/50 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800">
                                            ...
                                            <div className="flex gap-4">
                                                <button onClick={handleExportHTML} className="px-6 py-3 bg-white dark:bg-slate-700 text-blue-600 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 border border-blue-50 dark:border-blue-900 shadow-sm hover:shadow-md transition-all">
                                                    <Download size={16} /> Export HTML
                                                </button>
                                                <button onClick={() => window.print()} className="px-6 py-3 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-lg hover:bg-blue-700 transition-all">
                ...
                <div className="bg-gradient-to-br from-indigo-600 to-violet-800 rounded-[3rem] p-10 mt-12 flex items-center justify-between border border-white/10 shadow-3xl relative overflow-hidden group">
                    ...
                    <div className="flex gap-4 relative z-10">
                        ...
                        <button className="px-10 py-4 bg-white text-indigo-900 rounded-[1.5rem] text-[10px] font-black uppercase tracking-[0.2em] transition-all shadow-[0_20px_40px_rgba(0,0,0,0.2)] hover:scale-105 active:scale-95">
                            Publish Final SEF
                        </button>
                    </div>
                </div>
